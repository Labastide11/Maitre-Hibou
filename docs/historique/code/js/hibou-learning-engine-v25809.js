/* Maître Hibou V25.8.09 — moteur commun d'exercices */
(function(){
'use strict';
if(window.__hibouLearningEngineV25809)return;
window.__hibouLearningEngineV25809=true;
...
function renderLesson(){
 ...
 +'<div class="hle-actions"><button class="hle-primary" type="button" onclick="hibouLearningNextV25809()">🧩 J’essaie avec Maître Hibou</button><button class="hle-secondary" type="button" onclick="openHibouReadingV25763()">← Choisir une autre activité</button></div></div>');
}
...
function optionsHTML(ex){
 if(ex.type==='order'){
   ...
   return '<div class="hle-order-zone"><div class="hle-order-answer">'+(seq.length?seq.map((x,i)=>'<button type="button" onclick="hibouLearningRemoveChunkV25809('+i+')">'+esc(x.value)+'</button>').join('<span>→</span>'):'<em>Touche les morceaux dans l’ordre.</em>')+'</div><div class="hle-order-bank">'+remaining.map(x=>'<button type="button" onclick="hibouLearningAddChunkV25809(\''+String(x).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')">'+esc(x)+'</button>').join('')+'</div></div>';
 }
 return '<div class="hle-choice-grid">'+(ex.choices||[]).map(c=>'<button type="button" class="hle-choice" onclick="hibouLearningChooseV25809(\''+String(c).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')">'+esc(c)+'</button>').join('')+'</div>';
}
...
function renderExercise(){
 ...
 +'<div class="hle-actions">'+(ex.type==='order'?'<button class="hle-primary" type="button" onclick="hibouLearningValidateOrderV25809()">Vérifier</button>':'')+(!noHelp?'<button class="hle-help" type="button" onclick="hibouLearningHelpV25809()">💡 Aide</button>':'')+(rec.done?'<button class="hle-primary" type="button" onclick="hibouLearningNextExerciseV25809()">Continuer →</button>':'')+'</div>'
...
function renderSummary(){
 ...
 shell('<div class="hle-card hle-summary"><h3>🦉 Mon bilan</h3><div class="hle-status '+m.key+'"><span>'+m.icon+'</span><strong>'+esc(m.label)+'</strong></div><div class="hle-scoreboard"><div><strong>'+s.score+'/'+s.total+'</strong><span>Entraînement</span></div><div><strong>'+state.challengeScore+'/'+state.challengeTotal+'</strong><span>Défi</span></div><div><strong>'+state.helpUsed+'</strong><span>Aide'+(state.helpUsed>1?'s':'')+' utilisée'+(state.helpUsed>1?'s':'')+'</span></div></div><p class="hle-bilan-advice">'+esc(advice)+'</p><div class="hle-saved">✅ Cette activité est ajoutée à ton parcours.</div><div class="hle-actions"><button class="hle-primary" type="button" onclick="hibouLearningRestartV25809()">Refaire l’activité</button><button class="hle-secondary" type="button" onclick="openHibouReadingV25763()">Choisir une autre activité</button></div></div>');
}
function open(code){const a=activities()[code];if(!a)return false;reset(a);renderLesson();return true;}
window.hibouLearningEngineV25809={open,activities};
window.hibouLearningNextV25809=function(){if(state.stage==='lesson')advanceStage();};
window.hibouLearningChooseV25809=choose;
window.hibouLearningAddChunkV25809=addChunk;
window.hibouLearningRemoveChunkV25809=removeChunk;
window.hibouLearningValidateOrderV25809=validateOrder;
window.hibouLearningHelpV25809=help;
window.hibouLearningNextExerciseV25809=nextExercise;
window.hibouLearningRestartV25809=function(){if(state.activity){reset(state.activity);renderLesson();}};
})();