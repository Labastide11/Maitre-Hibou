(function(){
  ...
  function bindCurrentReadingOwlV25831(){
    const host=inner();
    if(!host) return;
    const owl=host.querySelector('.fr-v2348-owl');
    if(owl) bindLongPressV25831(owl);
  }
  ...
  function renderHome(){
    ...
    host.innerHTML=titleBlock()+'<div class="fr-v2348-content"><section class="fr-v2348-panel"><div class="fr-v25763-shell">'
      ...
      +'</div></section></div>';
    requestAnimationFrame(bindCurrentReadingOwlV25831);
  }
  ...
  window.finishHibouReadingMissionV25763=function(code){
    ...
    try{
      if(testMode){
        /* V25.8.31 : test enseignant, aucune trace élève. */
  ...
  /* V25.8.31 — Mode test enseignant Français : appui long 3 s sur le hibou. */
  ...
  function closeFrenchTeacherTestV25831(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25831');
    if(ov){
  ...
  function refreshTeacherTestPopupV25831(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25831');
    if(!ov) return;
  ...
  function ensureFrenchTeacherTestOverlayV25831(){
    let ov=document.getElementById('hibouFrenchTeacherTestOverlayV25831');
    if(ov) return ov;
    ov=document.createElement('div');
    ov.id='hibouFrenchTeacherTestOverlayV25831';
    ov.className='hibou-fr-test-overlay-v25831 hidden';
    ov.innerHTML=
      '<div class="hibou-fr-test-popup-v25831" role="dialog" aria-modal="true" aria-labelledby="hibouFrTestTitleV25831">'
      +'<div class="hibou-fr-test-head-v25831"><div><span>🧪</span><strong id="hibouFrTestTitleV25831">Test enseignant — Français</strong></div><button type="button" class="hibou-fr-test-close-v25831" aria-label="Fermer">×</button></div>'
      +'<p class="hibou-fr-test-intro-v25831">Choisis la période à afficher dans <strong>Lecture &amp; compréhension</strong>.</p>'
      +'<div class="hibou-fr-test-periods-v25831">'
      ...
      +'</div><div class="hibou-fr-test-status-v25831" data-fr-test-status></div>'
      +'<div class="hibou-fr-test-note-v25831">🔒 Ce réglage est local à cet appareil. Il ne modifie ni la période scolaire réelle ni le parcours des élèves.</div>'
      +'</div>';
    document.documentElement.appendChild(ov);
    ov.addEventListener('click',ev=>{if(ev.target===ov)closeFrenchTeacherTestV25831();});
    ov.querySelector('.hibou-fr-test-close-v25831').addEventListener('click',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25831();
    });
    ov.querySelector('.hibou-fr-test-close-v25831').addEventListener('pointerup',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25831();
    });
    ov.querySelectorAll('[data-fr-test-period]').forEach(btn=>{
      btn.addEventListener('click',()=>setFrenchTeacherTestPeriodV25831(Number(btn.getAttribute('data-fr-test-period')||0)));
    });
    return ov;
  }

  function openFrenchTeacherTestV25831(){
    const ov=ensureFrenchTeacherTestOverlayV25831();
    refreshTeacherTestPopupV25831();
    ov.classList.remove('hidden');
    ov.setAttribute('aria-hidden','false');
    ov.style.pointerEvents='auto';
    ov.style.zIndex='2147483646';
    const closeBtn=ov.querySelector('.hibou-fr-test-close-v25831');
    if(closeBtn){ try{ closeBtn.focus({preventScroll:true}); }catch(e){ closeBtn.focus(); } }
  }

  function cancelHoldV25831(){
    if(teacherHoldTimer){clearTimeout(teacherHoldTimer);teacherHoldTimer=null;}
  }

  function bindLongPressV25831(owl){
    if(!owl||owl.dataset.frTeacherHoldV25831==='1')return;
    owl.dataset.frTeacherHoldV25831='1';
  ...
  function enhanceFrenchTestUIV25831(){
    ...
    french.querySelectorAll('.fr-v2348-owl').forEach(bindLongPressV25831);
    bindCurrentReadingOwlV25831();

    /* V25.8.31 : le mode test reste entièrement invisible dans les vues élèves. */
    french.querySelectorAll('.hibou-fr-test-badge-v25831,.hibou-fr-test-badge-v25827,.hibou-fr-test-badge-v25826,.hibou-fr-test-badge-v25822')
      .forEach(el=>el.remove());
  }

  window.openFrenchTeacherTestV25831=openFrenchTeacherTestV25831;
  window.openFrenchTeacherTestV25830=openFrenchTeacherTestV25831;
  window.openFrenchTeacherTestV25829=openFrenchTeacherTestV25831;
  window.openFrenchTeacherTestV25828=openFrenchTeacherTestV25831;
  window.openFrenchTeacherTestV25827=openFrenchTeacherTestV25831;
  window.openFrenchTeacherTestV25826=openFrenchTeacherTestV25831;
  window.openFrenchTeacherTestV25822=openFrenchTeacherTestV25831;
  window.closeFrenchTeacherTestV25831=closeFrenchTeacherTestV25831;
  window.closeFrenchTeacherTestV25830=closeFrenchTeacherTestV25831;
  window.closeFrenchTeacherTestV25829=closeFrenchTeacherTestV25831;
  window.closeFrenchTeacherTestV25828=closeFrenchTeacherTestV25831;
  window.closeFrenchTeacherTestV25827=closeFrenchTeacherTestV25831;
  window.closeFrenchTeacherTestV25826=closeFrenchTeacherTestV25831;
  window.closeFrenchTeacherTestV25822=closeFrenchTeacherTestV25831;
  window.setFrenchTeacherTestPeriodV25831=setFrenchTeacherTestPeriodV25831;
  window.isHibouFrenchTestModeV25831=isFrenchTestMode;

  window.HIBOU_LECTURE_46={items:ITEMS,currentPeriod,periodFromDate,isTestMode:isFrenchTestMode,forcedTestPeriod};

  let uiRefreshQueuedV25831=false;
  function scheduleFrenchUiRefreshV25831(){
    if(uiRefreshQueuedV25831)return;
    uiRefreshQueuedV25831=true;
    requestAnimationFrame(()=>{
      uiRefreshQueuedV25831=false;
      ensureCard();
      enhanceFrenchTestUIV25831();
    });
  }
  const obs=new MutationObserver(mutations=>{
    ...
    const relevant=mutations.some(m=>{
      ...
      return !(t && (t.closest?.('.hibou-fr-test-overlay-v25831')));
    });
    if(relevant)scheduleFrenchUiRefreshV25831();
  });
  obs.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('keydown',ev=>{if(ev.key==='Escape')closeFrenchTeacherTestV25831();});

  function initV25831(){ensureCard();enhanceFrenchTestUIV25831();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initV25831,{once:true});else initV25831();
  [200,600,1200,2200].forEach(ms=>setTimeout(initV25831,ms));
})();
