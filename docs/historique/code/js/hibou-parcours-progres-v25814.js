/* Maître Hibou V25.8.14 — Mes progrès enrichi avec les traces pédagogiques */
(function(){
'use strict';if(window.__hibouProgressHubV25814)return;window.__hibouProgressHubV25814=true;
var VERSION='🦉 Maître Hibou V25.8.14',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
...
var QUESTION_STORE_V25814='hibou_student_questions_v25807';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25814)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25814,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.14 : source unique = Google Sheet via API V2.7.1.
...
function questionsApiConfigV25814(){
  try{
    return {
...
function questionsApiSnapshotV25814(){
  var cfg=questionsApiConfigV25814();
  if(!cfg.url){
...
function loadStudentSnapshot(force){
  ...
  return questionsApiSnapshotV25814().then(function(snap){
...
window.openHibouProgressHubV25814=open;window.closeHibouProgressHubV25814=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
