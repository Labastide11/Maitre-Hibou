/* Maître Hibou V25.8.16 — moteur commun d'exercices */
(function(){
'use strict';
if(window.__hibouLearningEngineV25816)return;
window.__hibouLearningEngineV25816=true;
...
function renderLesson(){
 ...
 +'<div class="hle-actions"><button class="hle-primary" type="button" onclick="hibouLearningNextV25816()">🧩 J’essaie avec Maître Hibou</button><button class="hle-secondary" type="button" onclick="openHibouReadingV25763()">← Choisir une autre activité</button></div></div>');
}
...
function optionsHTML(ex){
 if(ex.type==='highlight'){
   ...
   return '<div class="hle-highlight-list">'+(ex.items||[]).map((c,i)=>{
     ...
     return '<button type="button" class="hle-highlight'+active+'" onclick="hibouLearningToggleHighlightV25816('+i+')">'+esc(c)+'</button>';
   }).join('')+'</div>';
 }
 if(ex.type==='multi'){
   const rec=answerRecord(ex), selected=rec.selected||[];
   return '<div class="hle-choice-grid hle-multi-grid">'+(ex.items||[]).map((c,i)=>{
     const active=selected.indexOf(i)>=0?' selected':'';
     return '<button type="button" class="hle-choice hle-multi'+active+'" onclick="hibouLearningToggleMultiV25816('+i+')">'+esc(c)+'</button>';
   }).join('')+'</div>';
 }
 if(ex.type==='order'){
   ...
   return '<div class="hle-order-zone"><div class="hle-order-answer">'+(seq.length?seq.map((x,i)=>'<button type="button" onclick="hibouLearningRemoveChunkV25816('+i+')">'+esc(x.value)+'</button>').join('<span>→</span>'):'<em>Touche les morceaux dans l’ordre.</em>')+'</div><div class="hle-order-bank">'+remaining.map(x=>'<button type="button" onclick="hibouLearningAddChunkV25816(\''+String(x).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')">'+esc(x)+'</button>').join('')+'</div></div>';
 }
 return '<div class="hle-choice-grid">'+(ex.choices||[]).map(c=>'<button type="button" class="hle-choice" onclick="hibouLearningChooseV25816(\''+String(c).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')">'+esc(c)+'</button>').join('')+'</div>';
}
...
function renderExercise(){
 ...
 +'<div class="hle-actions">'+((ex.type==='order'||ex.type==='multi'||ex.type==='highlight')?'<button class="hle-primary" type="button" onclick="'+(ex.type==='multi'?'hibouLearningValidateMultiV25816()':ex.type==='highlight'?'hibouLearningValidateHighlightV25816()':'hibouLearningValidateOrderV25816()')+'">Vérifier</button>':'')+(!noHelp?'<button class="hle-help" type="button" onclick="hibouLearningHelpV25816()">💡 Aide</button>':'')+(rec.done?'<button class="hle-primary" type="button" onclick="hibouLearningNextExerciseV25816()">Continuer →</button>':'')+'</div>'
...
function renderSummary(){
 ...
 shell('<div class="hle-card hle-summary"><h3>🦉 Mon bilan</h3><div class="hle-status '+m.key+'"><span>'+m.icon+'</span><strong>'+esc(m.label)+'</strong></div><div class="hle-scoreboard"><div><strong>'+s.score+'/'+s.total+'</strong><span>Entraînement</span></div><div><strong>'+state.challengeScore+'/'+state.challengeTotal+'</strong><span>Défi</span></div><div><strong>'+state.helpUsed+'</strong><span>Aide'+(state.helpUsed>1?'s':'')+' utilisée'+(state.helpUsed>1?'s':'')+'</span></div></div><p class="hle-bilan-advice">'+esc(advice)+'</p><div class="hle-saved">✅ Cette activité est ajoutée à ton parcours.</div><div class="hle-actions"><button class="hle-primary" type="button" onclick="hibouLearningRestartV25816()">Refaire l’activité</button><button class="hle-secondary" type="button" onclick="openHibouReadingV25763()">Choisir une autre activité</button></div></div>');
}
function open(code){const a=activities()[code];if(!a)return false;reset(a);renderLesson();return true;}
window.hibouLearningEngineV25816={open,activities};
window.hibouLearningNextV25816=function(){if(state.stage==='lesson')advanceStage();};
window.hibouLearningChooseV25816=choose;
window.hibouLearningToggleHighlightV25816=toggleHighlight;
window.hibouLearningValidateHighlightV25816=validateHighlight;
window.hibouLearningToggleMultiV25816=toggleMulti;
window.hibouLearningValidateMultiV25816=validateMulti;
window.hibouLearningAddChunkV25816=addChunk;
window.hibouLearningRemoveChunkV25816=removeChunk;
window.hibouLearningValidateOrderV25816=validateOrder;
window.hibouLearningHelpV25816=help;
window.hibouLearningNextExerciseV25816=nextExercise;
window.hibouLearningRestartV25816=function(){if(state.activity){reset(state.activity);renderLesson();}};
})();