/* Maître Hibou V25.8.21 — Mes progrès enrichi + fermeture explicite */
(function(){
'use strict';if(window.__hibouProgressHubV25821)return;window.__hibouProgressHubV25821=true;
var VERSION='🦉 Maître Hibou V25.8.21',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
...
var QUESTION_STORE_V25821='hibou_student_questions_v25807';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25821)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25821,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.21 : source unique = Google Sheet via API V2.7.1.
...
function questionsApiConfigV25821(){
  try{
    return {
...
function questionsApiSnapshotV25821(){
  var cfg=questionsApiConfigV25821();
  if(!cfg.url){
...
function loadStudentSnapshot(force){
  ...
  return questionsApiSnapshotV25821().then(function(snap){
...
function renderPopup(){
  ...
  body.innerHTML=
    ...
    +'<div class="hp79-footer hp821-footer"><span>🦉 Continue à ton rythme : chaque petit progrès compte.</span><button type="button" class="hp821-close-btn" data-hp821-close>✕ Fermer</button></div>';
  ...
  var closeBtn=body.querySelector('[data-hp821-close]');
...
window.openHibouProgressHubV25821=open;window.closeHibouProgressHubV25821=close;window.openHibouProgressHubV25814=open;window.closeHibouProgressHubV25814=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
