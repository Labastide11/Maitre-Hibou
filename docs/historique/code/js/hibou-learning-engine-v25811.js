/* Maître Hibou V25.8.11 — moteur commun d'exercices */
(function(){
'use strict';
if(window.__hibouLearningEngineV25811)return;
window.__hibouLearningEngineV25811=true;
...
function renderLesson(){
 ...
 +'<div class="hle-actions"><button class="hle-primary" type="button" onclick="hibouLearningNextV25811()">🧩 J’essaie avec Maître Hibou</button><button class="hle-secondary" type="button" onclick="openHibouReadingV25763()">← Choisir une autre activité</button></div></div>');
}
...
function optionsHTML(ex){
 if(ex.type==='multi'){
   ...
   return '<div class="hle-choice-grid hle-multi-grid">'+(ex.items||[]).map((c,i)=>{
     ...
     return '<button type="button" class="hle-choice hle-multi'+active+'" onclick="hibouLearningToggleMultiV25811('+i+')">'+esc(c)+'</button>';
   }).join('')+'</div>';
 }
 if(ex.type==='order'){
   ...
   return '<div class="hle-order-zone"><div class="hle-order-answer">'+(seq.length?seq.map((x,i)=>'<button type="button" onclick="hibouLearningRemoveChunkV25811('+i+')">'+esc(x.value)+'</button>').join('<span>→</span>'):'<em>Touche les morceaux dans l’ordre.</em>')+'</div><div class="hle-order-bank">'+remaining.map(x=>'<button type="button" onclick="hibouLearningAddChunkV25811(\''+String(x).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')">'+esc(x)+'</button>').join('')+'</div></div>';
 }
 return '<div class="hle-choice-grid">'+(ex.choices||[]).map(c=>'<button type="button" class="hle-choice" onclick="hibouLearningChooseV25811(\''+String(c).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')">'+esc(c)+'</button>').join('')+'</div>';
}
...
function renderExercise(){
 ...
 +'<div class="hle-actions">'+((ex.type==='order'||ex.type==='multi')?'<button class="hle-primary" type="button" onclick="'+(ex.type==='multi'?'hibouLearningValidateMultiV25811()':'hibouLearningValidateOrderV25811()')+'">Vérifier</button>':'')+(!noHelp?'<button class="hle-help" type="button" onclick="hibouLearningHelpV25811()">💡 Aide</button>':'')+(rec.done?'<button class="hle-primary" type="button" onclick="hibouLearningNextExerciseV25811()">Continuer →</button>':'')+'</div>'
...
function renderSummary(){
 ...
 shell('<div class="hle-card hle-summary"><h3>🦉 Mon bilan</h3><div class="hle-status '+m.key+'"><span>'+m.icon+'</span><strong>'+esc(m.label)+'</strong></div><div class="hle-scoreboard"><div><strong>'+s.score+'/'+s.total+'</strong><span>Entraînement</span></div><div><strong>'+state.challengeScore+'/'+state.challengeTotal+'</strong><span>Défi</span></div><div><strong>'+state.helpUsed+'</strong><span>Aide'+(state.helpUsed>1?'s':'')+' utilisée'+(state.helpUsed>1?'s':'')+'</span></div></div><p class="hle-bilan-advice">'+esc(advice)+'</p><div class="hle-saved">✅ Cette activité est ajoutée à ton parcours.</div><div class="hle-actions"><button class="hle-primary" type="button" onclick="hibouLearningRestartV25811()">Refaire l’activité</button><button class="hle-secondary" type="button" onclick="openHibouReadingV25763()">Choisir une autre activité</button></div></div>');
}
function open(code){const a=activities()[code];if(!a)return false;reset(a);renderLesson();return true;}
window.hibouLearningEngineV25811={open,activities};
window.hibouLearningNextV25811=function(){if(state.stage==='lesson')advanceStage();};
window.hibouLearningChooseV25811=choose;
window.hibouLearningToggleMultiV25811=toggleMulti;
window.hibouLearningValidateMultiV25811=validateMulti;
window.hibouLearningAddChunkV25811=addChunk;
window.hibouLearningRemoveChunkV25811=removeChunk;
window.hibouLearningValidateOrderV25811=validateOrder;
window.hibouLearningHelpV25811=help;
window.hibouLearningNextExerciseV25811=nextExercise;
window.hibouLearningRestartV25811=function(){if(state.activity){reset(state.activity);renderLesson();}};
})();