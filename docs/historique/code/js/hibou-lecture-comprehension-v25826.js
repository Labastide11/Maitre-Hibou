(function(){
  ...
  function renderMission(code){
    ...
    try{
      if(window.hibouLearningEngineV25826 && window.HIBOU_LEARNING_ACTIVITIES && window.HIBOU_LEARNING_ACTIVITIES[code]){
        if(window.hibouLearningEngineV25826.open(code)) return;
      }
    }catch(e){}
  ...
  window.finishHibouReadingMissionV25763=function(code){
    ...
    try{
      if(testMode){
        /* V25.8.26 : test enseignant, aucune trace élève. */
  ...
  /* V25.8.26 — Mode test enseignant Français : appui long 3 s sur le hibou. */
  ...
  function closeFrenchTeacherTestV25826(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25826');
    if(ov){
  ...
  function refreshTeacherTestPopupV25826(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25826');
    if(!ov) return;
  ...
  function ensureFrenchTeacherTestOverlayV25826(){
    let ov=document.getElementById('hibouFrenchTeacherTestOverlayV25826');
    if(ov) return ov;
    ov=document.createElement('div');
    ov.id='hibouFrenchTeacherTestOverlayV25826';
    ov.className='hibou-fr-test-overlay-v25826 hidden';
    ov.innerHTML=
      '<div class="hibou-fr-test-popup-v25826" role="dialog" aria-modal="true" aria-labelledby="hibouFrTestTitleV25826">'
      +'<div class="hibou-fr-test-head-v25826"><div><span>🧪</span><strong id="hibouFrTestTitleV25826">Test enseignant — Français</strong></div><button type="button" class="hibou-fr-test-close-v25826" aria-label="Fermer">×</button></div>'
      +'<p class="hibou-fr-test-intro-v25826">Choisis la période à afficher dans <strong>Lecture &amp; compréhension</strong>.</p>'
      +'<div class="hibou-fr-test-periods-v25826">'
      ...
      +'</div><div class="hibou-fr-test-status-v25826" data-fr-test-status></div>'
      +'<div class="hibou-fr-test-note-v25826">🔒 Ce réglage est local à cet appareil. Il ne modifie ni la période scolaire réelle ni le parcours des élèves.</div>'
      +'</div>';
    document.documentElement.appendChild(ov);
    ov.addEventListener('click',ev=>{if(ev.target===ov)closeFrenchTeacherTestV25826();});
    ov.querySelector('.hibou-fr-test-close-v25826').addEventListener('click',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25826();
    });
    ov.querySelector('.hibou-fr-test-close-v25826').addEventListener('pointerup',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25826();
    });
    ov.querySelectorAll('[data-fr-test-period]').forEach(btn=>{
      btn.addEventListener('click',()=>setFrenchTeacherTestPeriodV25826(Number(btn.getAttribute('data-fr-test-period')||0)));
    });
    return ov;
  }

  function openFrenchTeacherTestV25826(){
    const ov=ensureFrenchTeacherTestOverlayV25826();
    refreshTeacherTestPopupV25826();
    ov.classList.remove('hidden');
    ov.setAttribute('aria-hidden','false');
    ov.style.pointerEvents='auto';
    ov.style.zIndex='2147483646';
    const closeBtn=ov.querySelector('.hibou-fr-test-close-v25826');
    if(closeBtn){ try{ closeBtn.focus({preventScroll:true}); }catch(e){ closeBtn.focus(); } }
  }

  function cancelHoldV25826(){
    if(teacherHoldTimer){clearTimeout(teacherHoldTimer);teacherHoldTimer=null;}
  }

  function bindLongPressV25826(owl){
    if(!owl||owl.dataset.frTeacherHoldV25826==='1')return;
    owl.dataset.frTeacherHoldV25826='1';
    owl.addEventListener('contextmenu',ev=>ev.preventDefault());
    const start=ev=>{
      if(ev.pointerType==='mouse'&&ev.button!==0)return;
      cancelHoldV25826();
      owl.classList.add('hibou-fr-secret-hold-v25826');
      teacherHoldTimer=setTimeout(()=>{
        teacherHoldTimer=null;
        owl.classList.remove('hibou-fr-secret-hold-v25826');
        try{navigator.vibrate&&navigator.vibrate(40);}catch(e){}
        openFrenchTeacherTestV25826();
      },3000);
    };
    const cancel=()=>{
      cancelHoldV25826();
  ...
  function enhanceFrenchTestUIV25826(){
    const french=document.getElementById('frenchTrainingOverlayV2348');
    if(!french)return;
    bindLongPressV25826(french.querySelector('.fr-v2348-owl'));

    let badge=french.querySelector('.hibou-fr-test-badge-v25826');
    const p=forcedTestPeriod();
    if(p){
      if(!badge){
        badge=document.createElement('div');
        badge.className='hibou-fr-test-badge-v25826';
        const popup=french.querySelector('.fr-v2348-popup');
        if(popup)popup.appendChild(badge);
      }
      badge.textContent='🧪 MODE TEST — P'+p;
      badge.hidden=false;
    }else if(badge){badge.hidden=true;}
  }

  window.openFrenchTeacherTestV25826=openFrenchTeacherTestV25826;
  window.openFrenchTeacherTestV25822=openFrenchTeacherTestV25826;
  window.closeFrenchTeacherTestV25826=closeFrenchTeacherTestV25826;
  window.closeFrenchTeacherTestV25822=closeFrenchTeacherTestV25826;
  window.setFrenchTeacherTestPeriodV25826=setFrenchTeacherTestPeriodV25826;
  window.isHibouFrenchTestModeV25826=isFrenchTestMode;

  window.HIBOU_LECTURE_46={items:ITEMS,currentPeriod,periodFromDate,isTestMode:isFrenchTestMode,forcedTestPeriod};

  const obs=new MutationObserver(()=>{ensureCard();enhanceFrenchTestUIV25826();});
  obs.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('keydown',ev=>{if(ev.key==='Escape')closeFrenchTeacherTestV25826();});

  function initV25826(){ensureCard();enhanceFrenchTestUIV25826();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initV25826,{once:true});else initV25826();
  [200,600,1200,2200].forEach(ms=>setTimeout(initV25826,ms));
})();
