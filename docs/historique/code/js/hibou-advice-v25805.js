/* Maître Hibou V25.8.05 — Conseil personnalisé et explicite
   Source : student_snapshot réel (API V2.7.4) + parcours local récent.
   Le conseil dit quoi faire, sur quoi, et pourquoi. */
(function(){
  'use strict';
  if(window.__hibouAdviceV25805) return;
  window.__hibouAdviceV25805 = true;
  ...
  function setRecommended(rec){
    ...
    Object.keys(dash).forEach(function(k){
      if(dash[k]) dash[k].classList.remove('v25805-recommended');
    });
    var key=rec.action==='questions'?'question':rec.action;
    if(dash[key]) dash[key].classList.add('v25805-recommended');
  }

  function openQuestions(){
    try{
      if(typeof window.openHibouProgressHubV25805==='function'){
        window.openHibouProgressHubV25805();
  ...
  function configureQuestionButton(rec){
    ...
    if(rec.action==='questions'){
      ...
      if(!btn.__v25805QuestionsBound){
        btn.__v25805QuestionsBound=true;
  ...
  function render(rec){
    ...
    if(box){
      box.innerHTML=
        '<div class="v25805-advice-action">'+esc(rec.title)+'</div>'+
        '<div class="v25805-advice-why">'+esc(rec.why)+'</div>';
    }
  ...
  function boot(){
    ...
    // V25.8.05 reprend donc la main après lui sans toucher à ses boutons.
