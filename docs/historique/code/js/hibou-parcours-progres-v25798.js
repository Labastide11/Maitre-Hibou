/* Maître Hibou V25.7.98 — Ceintures illustrées dans Mon parcours et mes progrès */
(function(){
'use strict';if(window.__hibouProgressHubV25798)return;window.__hibouProgressHubV25798=true;
var VERSION='🦉 Maître Hibou V25.7.98',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false;
var QUOTES=[{theme:'La confiance',text:'Je crois en toi.'},{theme:'La confiance',text:'J’ai confiance en toi.'},{theme:'La confiance',text:'Même quand c’est dur et que cela représente un vrai défi, tu peux puiser dans tes ressources pour y arriver, à ton rythme.'},{theme:'La confiance',text:'Tu es capable de réaliser bien plus que ce que tu crois.'},{theme:'La motivation',text:'Mets ton cœur dans tout ce que tu fais.'},{theme:'La motivation',text:'Tu n’es pas en train d’échouer, tu es en train d’apprendre.'},{theme:'Pour progresser',text:'Tu as le droit de demander de l’aide.'},{theme:'Pour progresser',text:'Ta valeur ne se résume pas à une note.'},{theme:'Pour progresser',text:'Tes erreurs sont des chances de progresser plus vite.'}];
...
function eventResult(e){if(!e)return'';var medal=clean(e.medaille||''),score=(e.score!==''&&e.score!=null&&e.total!==''&&e.total!=null)?clean(e.score)+'/'+clean(e.total):'';if(medal){var nice=/or/i.test(medal)?'🥇 Or':/argent/i.test(medal)?'🥈 Argent':/bronze/i.test(medal)?'🥉 Bronze':medal;return score?nice+' · '+score:nice}if(score)return score;if(e.resultat==='reussite')return'Réussite';if(e.resultat==='erreur')return'À reprendre';return''}
function eventRow(e){var dom=studentLabel(e.matiere||e.domaine||''),res=eventResult(e);return'<article class="hp79-event"><span class="hp79-event-icon">'+eventIcon(e)+'</span><div><strong>'+esc(eventTitle(e))+'</strong><small>'+esc(formatDate(e.date_iso||e.date||''))+(dom?' · '+esc(dom):'')+'</small>'+(res?'<span class="hp79-result">'+esc(res)+'</span>':'')+'</div></article>'}
...
var QUESTION_STORE_V25798='hibou_student_questions_v25798';
...
function questionStatusLabel(v){
  ...
  if(/a_expliquer|à expliquer|expliquer|attente/.test(s))return'À expliquer';
  return'Envoyée';
}
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25798)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25798,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  var all=[], nameKey=norm(currentName());

  try{
    var snap=(latestSnapshot&&Array.isArray(latestSnapshot.questions))?latestSnapshot.questions:[];
    snap.forEach(function(q){
      var n=norm(q.prenom||q.student||q.eleve||currentName());
      if(!n||n===nameKey)all.push(q);
    });
  }catch(e){}

  readStoredQuestions().forEach(function(q){
    var n=norm(q.prenom||q.student||q.eleve||currentName());
    if(!n||n===nameKey)all.push(q);
  });
  ...
  history().forEach(function(e){
    if(!e)return;
    var s=norm([e.type,e.matiere,e.domaine].join(' '));
    if(/question|boite_a_questions/.test(s)){
      all.push({
        prenom:currentName(),
        question:e.questionCorrigee||e.question||e.texte||e.titre||e.activite,
        date_iso:e.date_iso||e.date,
        statut:e.statut||e.status||'nouvelle'
      });
    }
  });
  ...
  all.sort(function(a,b){
    return String(questionDate(b)||'').localeCompare(String(questionDate(a)||''));
  });
  return all;
}
...
function questionsHTML(){
  ...
  if(!rows.length){
    return '<div class="hp79-empty hp96-question-empty">Tu n’as pas encore posé de question. Quand tu en poseras une à Maître Hibou, elle apparaîtra ici. 💬</div>';
  }
  return '<div class="hp96-question-list">'+rows.map(questionRowHTML).join('')+'</div>';
}

function successes(){return history().filter(function(e){return e&&(e.resultat==='reussite'||/ceinture.*validee/.test(clean(e.type))||rank(e.medaille)>0)}).slice(0,8)}
...
function calcRecords(){var out=[],snap=(latestSnapshot&&Array.isArray(latestSnapshot.records))?latestSnapshot.records:[];snap.forEach(function(r){out.push({score:Number(r.score)||0,total:Number(r.total)||10,time:Number(r.temps_secondes||r.temps||0)||0,date:r.date||''})});history().forEach(function(e){if(!e)return;var maths=/math/i.test(clean(e.matiere)),train=/entrainement/.test(clean(e.type));if(maths&&train&&e.score!==''&&e.score!=null)out.push({score:Number(e.score)||0,total:Number(e.total)||10,time:Number(e.temps_secondes)||0,date:e.date_iso||e.date||''})});var seen={};out=out.filter(function(r){var k=[r.score,r.total,r.time,r.date].join('|');if(seen[k])return false;seen[k]=1;return true});out.sort(function(a,b){return String(b.date||'').localeCompare(String(a.date||''))});return out}
...
function personalCalc(){var rows=calcRecords();if(!rows.length)return{last:null,best:null,bestTime:0};var times=rows.map(function(r){return r.time}).filter(function(t){return t>0});return{last:rows[0],best:bestRecord(rows),bestTime:times.length?Math.min.apply(Math,times):0}}
...
function calcHTML(){var p=personalCalc();if(!p.last)return'<div class="hp79-empty">Aucun entraînement de calcul mental enregistré pour le moment.</div>';var cb=classBest(),classCard=classRecordsLoading?'<div class="hp79-calc-card class-record loading"><span>🏆 Record de la classe</span><strong>Chargement…</strong></div>':'<div class="hp79-calc-card class-record"><span>🏆 Record de la classe</span><strong>'+(cb?esc(cb.score)+'/'+esc(cb.total):'—')+'</strong></div>';return'<div class="hp79-calc"><div class="hp79-calc-card"><span>Mon dernier score</span><strong>'+esc(p.last.score)+'/'+esc(p.last.total)+'</strong></div><div class="hp79-calc-card"><span>Mon meilleur score</span><strong>'+esc(p.best.score)+'/'+esc(p.best.total)+'</strong></div><div class="hp79-calc-card"><span>Mon meilleur temps</span><strong>'+(p.bestTime?esc(p.bestTime)+' s':'—')+'</strong></div>'+classCard+'</div><p class="hp79-note">Le record de la classe est seulement un repère pour voir jusqu’où on peut aller. Aucun prénom n’est affiché.</p>'}
function objectiveText(){var retry=history().find(function(e){return e&&e.resultat==='erreur'});if(retry)return'Je reprends tranquillement : '+eventTitle(retry)+'.';if(!bestCompetences().length)return'Je m’entraîne, puis j’essaie ma première ceinture quand je me sens prêt.';return'Je continue à m’entraîner régulièrement et j’avance à mon rythme.'}
function prideText(){var h=history(),belt=h.find(function(e){return e&&/ceinture.*validee/.test(clean(e.type))}),success=belt||h.find(function(e){return e&&e.resultat==='reussite'});if(success)return eventTitle(success)+(eventResult(success)?' — '+eventResult(success):'');var b=bestCompetences();if(b.length)return studentLabel(b[0].competence||'Une compétence validée')+' — '+clean(b[0].medaille||'');return'Chaque activité terminée est déjà un pas en avant.'}
...
function renderPopup(){
  ...
  body.innerHTML=
    ...
    +'<div class="hp79-footer">🦉 Continue à ton rythme : chaque petit progrès compte.</div>';
...
function open(ev){if(ev&&ev.preventDefault)ev.preventDefault();quoteIndex=chooseQuote();activeTab='recent';recentExpanded=false;renderPopup();ensureOverlay().classList.remove('hidden');try{if(window.hibouRefreshStudent)window.hibouRefreshStudent(true).then(function(s){if(s){latestSnapshot=s;renderHeaderCard();renderPopup()}})}catch(e){}}
function close(){if(overlay)overlay.classList.add('hidden')}function bindTopCard(){var card=document.getElementById('bandeauLastCard');if(!card||card.__hp79Bound)return;card.__hp79Bound=true;card.addEventListener('click',open,true);card.addEventListener('keydown',function(ev){if(ev.key==='Enter'||ev.key===' '){ev.preventDefault();open(ev)}},true)}
function refresh(force){renderHeaderCard();bindTopCard();try{if(window.hibouRefreshStudent)return window.hibouRefreshStudent(!!force).then(function(s){if(s)latestSnapshot=s;renderHeaderCard();if(overlay&&!overlay.classList.contains('hidden'))renderPopup();return s})}catch(e){}return Promise.resolve(null)}
...
window.openHibouProgressHubV25798=open;window.closeHibouProgressHubV25798=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
window.addEventListener('message',function(ev){
  var d=ev&&ev.data;
  if(!d)return;
  if(d.type==='hibou_question_posee'||d.type==='question_posee'||d.event_type==='hibou_question_posee'){
    writeStoredQuestion(d);
    if(overlay&&!overlay.classList.contains('hidden')&&activeTab==='questions')renderPanel('questions');
  }
});
document.addEventListener('hibou:question-saved',function(ev){
  if(ev&&ev.detail){
    writeStoredQuestion(ev.detail);
    if(overlay&&!overlay.classList.contains('hidden')&&activeTab==='questions')renderPanel('questions');
  }
});
