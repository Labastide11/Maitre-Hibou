/* Maître Hibou V25.8.01 — Ceintures illustrées dans Mon parcours et mes progrès */
(function(){
'use strict';if(window.__hibouProgressHubV25801)return;window.__hibouProgressHubV25801=true;
var VERSION='🦉 Maître Hibou V25.8.01',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
...
var QUESTION_STORE_V25801='hibou_student_questions_v25801';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25801)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25801,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.01 : source unique = Google Sheet via API V2.7.1.
...
function existingStudentLoader(force){
  var name=currentName();

  // Priorité au système élève historique de Maître Hibou :
  // il possède déjà l'URL API, la clé appareil et la logique d'authentification.
  try{
    if(window.hibouStudentSystem&&typeof window.hibouStudentSystem.loadSnapshot==='function'){
      return Promise.resolve(window.hibouStudentSystem.loadSnapshot(name,!!force));
    }
  }catch(e){}

  try{
    if(typeof window.hibouRefreshStudent==='function'){
      return Promise.resolve(window.hibouRefreshStudent(!!force));
    }
  }catch(e){}

  return Promise.reject(new Error('Système de synchronisation élève indisponible'));
}
...
function loadStudentSnapshot(force){
  ...
  return existingStudentLoader(!!force).then(function(snap){
    ...
    // API V2.7.1 doit fournir questions_meta + questions.
    if(activeTab==='questions'){
      if(!latestSnapshot||!latestSnapshot.questions_meta){
        questionsError='La synchronisation des questions n’a pas répondu avec les données attendues.';
      }else if(latestSnapshot.questions_meta.source!=='sheet'){
        questionsError='La source des questions n’est pas le Google Sheet.';
      }else{
        questionsError='';
      }
...
window.openHibouProgressHubV25801=open;window.closeHibouProgressHubV25801=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
