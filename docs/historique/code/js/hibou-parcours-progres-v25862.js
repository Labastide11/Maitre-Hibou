...
(function(){
'use strict';if(window.__hibouProgressHubV25862)return;window.__hibouProgressHubV25862=true;
var VERSION='🦉 Maître Hibou V25.8.62',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
var QUOTES=[
...
function eventRow(e){if(isBeltTraceV25858(e))return beltTraceRowV25858(e);if(isLearningTrace(e))return learningEventRow(e);var dom=studentLabel(e.matiere||e.domaine||''),res=eventResult(e);return'<article class="hp79-event"><span class="hp79-event-icon">'+eventIcon(e)+'</span><div><strong>'+esc(eventTitle(e))+'</strong><small>'+esc(formatDate(e.date_iso||e.date||''))+(dom?' · '+esc(dom):'')+'</small>'+(res?'<span class="hp79-result">'+esc(res)+'</span>':'')+'</div></article>'}
