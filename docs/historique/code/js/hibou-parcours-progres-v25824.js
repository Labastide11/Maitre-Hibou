/* Maître Hibou V25.8.24 — Mes progrès enrichi + fermeture explicite */
(function(){
'use strict';if(window.__hibouProgressHubV25824)return;window.__hibouProgressHubV25824=true;
var VERSION='🦉 Maître Hibou V25.8.24',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
...
var QUESTION_STORE_V25824='hibou_student_questions_v25807';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25824)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25824,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.24 : source unique = Google Sheet via API V2.7.1.
...
function questionsApiConfigV25824(){
  try{
    return {
...
function questionsApiSnapshotV25824(){
  var cfg=questionsApiConfigV25824();
  if(!cfg.url){
...
function loadStudentSnapshot(force){
  ...
  return questionsApiSnapshotV25824().then(function(snap){
