...
(function(){
  'use strict';
  if(window.__hibouAdviceV25804) return;
  window.__hibouAdviceV25804 = true;
  ...
  function setRecommended(rec){
    ...
    Object.keys(dash).forEach(function(k){
      if(dash[k]) dash[k].classList.remove('v25804-recommended');
    });
    var key=rec.action==='questions'?'question':rec.action;
    if(dash[key]) dash[key].classList.add('v25804-recommended');
  }

  function openQuestions(){
    try{
      if(typeof window.openHibouProgressHubV25804==='function'){
        window.openHibouProgressHubV25804();
  ...
  function configureQuestionButton(rec){
    ...
    if(rec.action==='questions'){
      ...
      if(!btn.__v25804QuestionsBound){
        btn.__v25804QuestionsBound=true;
  ...
  function render(rec){
    ...
    if(box){
      box.innerHTML=
        '<div class="v25804-advice-action">'+esc(rec.title)+'</div>'+
        '<div class="v25804-advice-why">'+esc(rec.why)+'</div>';
    }
  ...
  function boot(){
    ...
    // V25.8.04 reprend donc la main après lui sans toucher à ses boutons.
