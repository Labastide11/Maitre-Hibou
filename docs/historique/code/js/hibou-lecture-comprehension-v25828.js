(function(){
  ...
  function renderMission(code){
    ...
    try{
      if(window.hibouLearningEngineV25828 && window.HIBOU_LEARNING_ACTIVITIES && window.HIBOU_LEARNING_ACTIVITIES[code]){
        if(window.hibouLearningEngineV25828.open(code)) return;
      }
    }catch(e){}
  ...
  window.finishHibouReadingMissionV25763=function(code){
    ...
    try{
      if(testMode){
        /* V25.8.28 : test enseignant, aucune trace élève. */
  ...
  /* V25.8.28 — Mode test enseignant Français : appui long 3 s sur le hibou. */
  ...
  function closeFrenchTeacherTestV25828(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25828');
    if(ov){
  ...
  function refreshTeacherTestPopupV25828(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25828');
    if(!ov) return;
  ...
  function ensureFrenchTeacherTestOverlayV25828(){
    let ov=document.getElementById('hibouFrenchTeacherTestOverlayV25828');
    if(ov) return ov;
    ov=document.createElement('div');
    ov.id='hibouFrenchTeacherTestOverlayV25828';
    ov.className='hibou-fr-test-overlay-v25828 hidden';
    ov.innerHTML=
      '<div class="hibou-fr-test-popup-v25828" role="dialog" aria-modal="true" aria-labelledby="hibouFrTestTitleV25828">'
      +'<div class="hibou-fr-test-head-v25828"><div><span>🧪</span><strong id="hibouFrTestTitleV25828">Test enseignant — Français</strong></div><button type="button" class="hibou-fr-test-close-v25828" aria-label="Fermer">×</button></div>'
      +'<p class="hibou-fr-test-intro-v25828">Choisis la période à afficher dans <strong>Lecture &amp; compréhension</strong>.</p>'
      +'<div class="hibou-fr-test-periods-v25828">'
      ...
      +'</div><div class="hibou-fr-test-status-v25828" data-fr-test-status></div>'
      +'<div class="hibou-fr-test-note-v25828">🔒 Ce réglage est local à cet appareil. Il ne modifie ni la période scolaire réelle ni le parcours des élèves.</div>'
      +'</div>';
    document.documentElement.appendChild(ov);
    ov.addEventListener('click',ev=>{if(ev.target===ov)closeFrenchTeacherTestV25828();});
    ov.querySelector('.hibou-fr-test-close-v25828').addEventListener('click',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25828();
    });
    ov.querySelector('.hibou-fr-test-close-v25828').addEventListener('pointerup',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25828();
    });
    ov.querySelectorAll('[data-fr-test-period]').forEach(btn=>{
      btn.addEventListener('click',()=>setFrenchTeacherTestPeriodV25828(Number(btn.getAttribute('data-fr-test-period')||0)));
    });
    return ov;
  }

  function openFrenchTeacherTestV25828(){
    const ov=ensureFrenchTeacherTestOverlayV25828();
    refreshTeacherTestPopupV25828();
    ov.classList.remove('hidden');
    ov.setAttribute('aria-hidden','false');
    ov.style.pointerEvents='auto';
    ov.style.zIndex='2147483646';
    const closeBtn=ov.querySelector('.hibou-fr-test-close-v25828');
    if(closeBtn){ try{ closeBtn.focus({preventScroll:true}); }catch(e){ closeBtn.focus(); } }
  }

  function cancelHoldV25828(){
    if(teacherHoldTimer){clearTimeout(teacherHoldTimer);teacherHoldTimer=null;}
  }

  function bindLongPressV25828(owl){
    if(!owl||owl.dataset.frTeacherHoldV25828==='1')return;
    owl.dataset.frTeacherHoldV25828='1';
    owl.addEventListener('contextmenu',ev=>ev.preventDefault());
    const start=ev=>{
      if(ev.pointerType==='mouse'&&ev.button!==0)return;
      cancelHoldV25828();
      owl.classList.add('hibou-fr-secret-hold-v25828');
      teacherHoldTimer=setTimeout(()=>{
        teacherHoldTimer=null;
        owl.classList.remove('hibou-fr-secret-hold-v25828');
        try{navigator.vibrate&&navigator.vibrate(40);}catch(e){}
        openFrenchTeacherTestV25828();
      },3000);
    };
    const cancel=()=>{
      cancelHoldV25828();
  ...
  function enhanceFrenchTestUIV25828(){
    const french=document.getElementById('frenchTrainingOverlayV2348');
    if(!french)return;
    bindLongPressV25828(french.querySelector('.fr-v2348-owl'));

    /* V25.8.28 : le mode test reste entièrement invisible dans la popup Français. */
    const oldBadge=french.querySelector('.hibou-fr-test-badge-v25828,.hibou-fr-test-badge-v25827,.hibou-fr-test-badge-v25826,.hibou-fr-test-badge-v25822');
    if(oldBadge) oldBadge.remove();
  }

  window.openFrenchTeacherTestV25828=openFrenchTeacherTestV25828;
  window.openFrenchTeacherTestV25827=openFrenchTeacherTestV25828;
  window.openFrenchTeacherTestV25826=openFrenchTeacherTestV25828;
  window.openFrenchTeacherTestV25822=openFrenchTeacherTestV25828;
  window.closeFrenchTeacherTestV25828=closeFrenchTeacherTestV25828;
  window.closeFrenchTeacherTestV25827=closeFrenchTeacherTestV25828;
  window.closeFrenchTeacherTestV25826=closeFrenchTeacherTestV25828;
  window.closeFrenchTeacherTestV25822=closeFrenchTeacherTestV25828;
  window.setFrenchTeacherTestPeriodV25828=setFrenchTeacherTestPeriodV25828;
  window.isHibouFrenchTestModeV25828=isFrenchTestMode;

  window.HIBOU_LECTURE_46={items:ITEMS,currentPeriod,periodFromDate,isTestMode:isFrenchTestMode,forcedTestPeriod};

  let uiRefreshQueuedV25828=false;
  function scheduleFrenchUiRefreshV25828(){
    if(uiRefreshQueuedV25828)return;
    uiRefreshQueuedV25828=true;
    requestAnimationFrame(()=>{
      uiRefreshQueuedV25828=false;
      ensureCard();
      enhanceFrenchTestUIV25828();
    });
  }
  const obs=new MutationObserver(mutations=>{
    ...
    const relevant=mutations.some(m=>{
      ...
      return !(t && (t.closest?.('.hibou-fr-test-overlay-v25828')));
    });
    if(relevant)scheduleFrenchUiRefreshV25828();
  });
  obs.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('keydown',ev=>{if(ev.key==='Escape')closeFrenchTeacherTestV25828();});

  function initV25828(){ensureCard();enhanceFrenchTestUIV25828();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initV25828,{once:true});else initV25828();
  [200,600,1200,2200].forEach(ms=>setTimeout(initV25828,ms));
})();
