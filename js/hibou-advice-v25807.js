/* Maître Hibou V25.8.07 — Conseil personnalisé et explicite
   Source : student_snapshot réel (API V2.7.4) + parcours local récent.
   Le conseil dit quoi faire, sur quoi, et pourquoi. */
(function(){
  'use strict';
  if(window.__hibouAdviceV25807) return;
  window.__hibouAdviceV25807 = true;
  ...
  function setRecommended(rec){
    ...
    Object.keys(dash).forEach(function(k){
      if(dash[k]) dash[k].classList.remove('v25807-recommended');
    });
    var key=rec.action==='questions'?'question':rec.action;
    if(dash[key]) dash[key].classList.add('v25807-recommended');
  }

  function openQuestions(){
    try{
      if(typeof window.openHibouProgressHubV25807==='function'){
        window.openHibouProgressHubV25807();
  ...
  function configureQuestionButton(rec){
    ...
    if(rec.action==='questions'){
      ...
      if(!btn.__v25807QuestionsBound){
        btn.__v25807QuestionsBound=true;
