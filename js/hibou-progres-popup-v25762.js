...
(function(){try{var st=document.createElement('style');st.id='hibouProgressPopupAvatarStyleV25768';st.textContent='.hcp-avatar{width:72px;height:72px;object-fit:contain;border-radius:18px;background:#fff6f4;border:2px solid #f0e5ff;padding:8px;}';document.head.appendChild(st);}catch(e){}})();
...
function currentStudentData(){
  try{
    if(latest&&latest.eleve&&latest.eleve.prenom) return latest.eleve;
  }catch(e){}
  try{
    var obj=JSON.parse(localStorage.getItem('hibou_current_student')||'{}');
    if(obj&&obj.prenom) return obj;
  }catch(e){}
  return {prenom:current()||'Élève',sexe:''};
}
function avatarPath(){
...
function avatarHtml(){
  var n=currentStudentData().prenom||current()||'Élève';
  return '<img class="hcp-avatar" src="'+avatarPath()+'" alt="Portrait de '+esc(n)+'">';
}
...
body.innerHTML='<header class="hcp-header">'+avatarHtml()+'<div><span class="hcp-chip">📊 Mes progrès</span><h2>'+esc(n)+'</h2><p>Mon parcours réel dans Maître Hibou</p></div></header>'+
...
function open(ev){if(ev&&ev.preventDefault)ev.preventDefault();render();ensure().classList.remove('hidden');document.body.classList.add('hibou-progress-popup-open');try{if(window.hibouRefreshStudent)window.hibouRefreshStudent(true).then(function(snap){if(snap)latest=snap;render();});}catch(e){}}
