/* Maître Hibou V25.8.05 — Ceintures illustrées dans Mon parcours et mes progrès */
(function(){
'use strict';if(window.__hibouProgressHubV25805)return;window.__hibouProgressHubV25805=true;
var VERSION='🦉 Maître Hibou V25.8.05',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
...
var QUESTION_STORE_V25805='hibou_student_questions_v25805';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25805)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25805,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.05 : source unique = Google Sheet via API V2.7.1.
...
function questionsApiConfigV25805(){
  try{
    return {
...
function questionsApiSnapshotV25805(){
  var cfg=questionsApiConfigV25805();
  if(!cfg.url){
...
function loadStudentSnapshot(force){
  ...
  return questionsApiSnapshotV25805().then(function(snap){
...
window.openHibouProgressHubV25805=open;window.closeHibouProgressHubV25805=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
