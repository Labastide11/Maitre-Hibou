/*
 * Maître Hibou V25.8.34 — Journal élève consolidé + codes canoniques LSU
...
(function(){
  'use strict';
  if(window.__hibouStudentSystemV25834) return;
  window.__hibouStudentSystemV25834=true;

  var VERSION='V25.8.34';
  ...
  /* V25.8.34 — raccordement prudent aux codes canoniques Progressions CE2.
  ...
  function canonicalSkillV25834(raw){
    raw=raw||{};
    ...
    if(skill&&CANONICAL_SKILLS_V25834[skill])return CANONICAL_SKILLS_V25834[skill];
    var hay=norm([raw.titre,raw.title,raw.detail,raw.details,raw.competence,raw.label,belt.label].filter(Boolean).join(' | '));
    for(var i=0;i<CANONICAL_LABELS_V25834.length;i++){
      if(hay.indexOf(norm(CANONICAL_LABELS_V25834[i][0]))!==-1){
        return CANONICAL_SKILLS_V25834[CANONICAL_LABELS_V25834[i][1]]||null;
      }
    }
    return null;
  }

  function enrichCanonicalV25834(raw,type,score,total){
    raw=raw||{};
    var meta=canonicalSkillV25834(raw);
  ...
  function normalizeEvent(raw){
    ...
    raw=enrichCanonicalV25834(raw,type,score,total);
  ...
  window.hibouTrackLearningActivity=trackLearningActivity;window.hibouCanonicalSkillV25834=canonicalSkillV25834;window.hibouStudentSystem={version:VERSION,track:track,trackLearningActivity:trackLearningActivity,canonicalSkill:canonicalSkillV25834,success:recordSuccess,error:recordError,currentName:currentName,getHistory:getHistory,getLast:getLast,loadSnapshot:loadSnapshot,refresh:refreshStudent,configured:configured,config:config};
