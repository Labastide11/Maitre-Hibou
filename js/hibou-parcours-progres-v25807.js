/* Maître Hibou V25.8.07 — Ceintures illustrées dans Mon parcours et mes progrès */
(function(){
'use strict';if(window.__hibouProgressHubV25807)return;window.__hibouProgressHubV25807=true;
var VERSION='🦉 Maître Hibou V25.8.07',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
...
var QUESTION_STORE_V25807='hibou_student_questions_v25807';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25807)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25807,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.07 : source unique = Google Sheet via API V2.7.1.
...
function questionsApiConfigV25807(){
  try{
    return {
...
function questionsApiSnapshotV25807(){
  var cfg=questionsApiConfigV25807();
  if(!cfg.url){
...
function loadStudentSnapshot(force){
  ...
  return questionsApiSnapshotV25807().then(function(snap){
...
window.openHibouProgressHubV25807=open;window.closeHibouProgressHubV25807=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
