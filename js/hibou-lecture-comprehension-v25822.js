(function(){
  ...
  window.finishHibouReadingMissionV25763=function(code){
    ...
    try{
      if(testMode){
        /* V25.8.22 : test enseignant, aucune trace élève. */
  ...
  /* V25.8.22 — Mode test enseignant Français : appui long 3 s sur le hibou. */
  ...
  function closeFrenchTeacherTestV25822(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25822');
    if(ov) ov.classList.add('hidden');
  }

  function refreshTeacherTestPopupV25822(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25822');
    if(!ov) return;
  ...
  function ensureFrenchTeacherTestOverlayV25822(){
    let ov=document.getElementById('hibouFrenchTeacherTestOverlayV25822');
    if(ov) return ov;
    ov=document.createElement('div');
    ov.id='hibouFrenchTeacherTestOverlayV25822';
    ov.className='hibou-fr-test-overlay-v25822 hidden';
    ov.innerHTML=
      '<div class="hibou-fr-test-popup-v25822" role="dialog" aria-modal="true" aria-labelledby="hibouFrTestTitleV25822">'
      +'<div class="hibou-fr-test-head-v25822"><div><span>🧪</span><strong id="hibouFrTestTitleV25822">Test enseignant — Français</strong></div><button type="button" class="hibou-fr-test-close-v25822" aria-label="Fermer">×</button></div>'
      +'<p class="hibou-fr-test-intro-v25822">Choisis la période à afficher dans <strong>Lecture &amp; compréhension</strong>.</p>'
      +'<div class="hibou-fr-test-periods-v25822">'
      ...
      +'</div><div class="hibou-fr-test-status-v25822" data-fr-test-status></div>'
      +'<div class="hibou-fr-test-note-v25822">🔒 Ce réglage est local à cet appareil. Il ne modifie ni la période scolaire réelle ni le parcours des élèves.</div>'
      +'</div>';
    document.body.appendChild(ov);
    ov.addEventListener('click',ev=>{if(ev.target===ov)closeFrenchTeacherTestV25822();});
    ov.querySelector('.hibou-fr-test-close-v25822').addEventListener('click',closeFrenchTeacherTestV25822);
    ov.querySelectorAll('[data-fr-test-period]').forEach(btn=>{
      btn.addEventListener('click',()=>setFrenchTeacherTestPeriodV25822(Number(btn.getAttribute('data-fr-test-period')||0)));
    });
    return ov;
  }

  function openFrenchTeacherTestV25822(){
    const ov=ensureFrenchTeacherTestOverlayV25822();
    refreshTeacherTestPopupV25822();
    ov.classList.remove('hidden');
  }

  function cancelHoldV25822(){
    if(teacherHoldTimer){clearTimeout(teacherHoldTimer);teacherHoldTimer=null;}
  }

  function bindLongPressV25822(owl){
    if(!owl||owl.dataset.frTeacherHoldV25822==='1')return;
    owl.dataset.frTeacherHoldV25822='1';
    owl.addEventListener('contextmenu',ev=>ev.preventDefault());
    const start=ev=>{
      if(ev.pointerType==='mouse'&&ev.button!==0)return;
      cancelHoldV25822();
      owl.classList.add('hibou-fr-secret-hold-v25822');
      teacherHoldTimer=setTimeout(()=>{
        teacherHoldTimer=null;
        owl.classList.remove('hibou-fr-secret-hold-v25822');
        try{navigator.vibrate&&navigator.vibrate(40);}catch(e){}
        openFrenchTeacherTestV25822();
      },3000);
    };
    const cancel=()=>{
      cancelHoldV25822();
  ...
  function enhanceFrenchTestUIV25822(){
    const french=document.getElementById('frenchTrainingOverlayV2348');
    if(!french)return;
    bindLongPressV25822(french.querySelector('.fr-v2348-owl'));

    let badge=french.querySelector('.hibou-fr-test-badge-v25822');
    const p=forcedTestPeriod();
    if(p){
      if(!badge){
        badge=document.createElement('div');
        badge.className='hibou-fr-test-badge-v25822';
  ...
  window.openFrenchTeacherTestV25822=openFrenchTeacherTestV25822;
  window.closeFrenchTeacherTestV25822=closeFrenchTeacherTestV25822;
  window.setFrenchTeacherTestPeriodV25822=setFrenchTeacherTestPeriodV25822;
  window.isHibouFrenchTestModeV25822=isFrenchTestMode;

  window.HIBOU_LECTURE_46={items:ITEMS,currentPeriod,periodFromDate,isTestMode:isFrenchTestMode,forcedTestPeriod};

  const obs=new MutationObserver(()=>{ensureCard();enhanceFrenchTestUIV25822();});
  obs.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('keydown',ev=>{if(ev.key==='Escape')closeFrenchTeacherTestV25822();});

  function initV25822(){ensureCard();enhanceFrenchTestUIV25822();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initV25822,{once:true});else initV25822();
  [200,600,1200,2200].forEach(ms=>setTimeout(initV25822,ms));
})();
