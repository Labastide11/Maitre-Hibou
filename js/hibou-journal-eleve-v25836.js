/*
 * Maître Hibou V25.8.36 — Journal élève consolidé + codes canoniques LSU
...
(function(){
  'use strict';
  if(window.__hibouStudentSystemV25836) return;
  window.__hibouStudentSystemV25836=true;

  var VERSION='V25.8.36';
  ...
  /* V25.8.36 — raccordement prudent aux codes canoniques Progressions CE2.
  ...
  function canonicalSkillV25836(raw){
    raw=raw||{};
    ...
    if(skill&&CANONICAL_SKILLS_V25836[skill])return CANONICAL_SKILLS_V25836[skill];
    var hay=norm([raw.titre,raw.title,raw.detail,raw.details,raw.competence,raw.label,belt.label].filter(Boolean).join(' | '));
    for(var i=0;i<CANONICAL_LABELS_V25836.length;i++){
      if(hay.indexOf(norm(CANONICAL_LABELS_V25836[i][0]))!==-1){
        return CANONICAL_SKILLS_V25836[CANONICAL_LABELS_V25836[i][1]]||null;
      }
    }
    return null;
  }

  function enrichCanonicalV25836(raw,type,score,total){
    raw=raw||{};
    var meta=canonicalSkillV25836(raw);
  ...
  function normalizeEvent(raw){
    ...
    raw=enrichCanonicalV25836(raw,type,score,total);
  ...
  window.hibouTrackLearningActivity=trackLearningActivity;window.hibouCanonicalSkillV25836=canonicalSkillV25836;window.hibouStudentSystem={version:VERSION,track:track,trackLearningActivity:trackLearningActivity,canonicalSkill:canonicalSkillV25836,success:recordSuccess,error:recordError,currentName:currentName,getHistory:getHistory,getLast:getLast,loadSnapshot:loadSnapshot,refresh:refreshStudent,configured:configured,config:config};
