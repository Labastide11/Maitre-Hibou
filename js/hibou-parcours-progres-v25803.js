/* Maître Hibou V25.8.03 — Ceintures illustrées dans Mon parcours et mes progrès */
(function(){
'use strict';if(window.__hibouProgressHubV25803)return;window.__hibouProgressHubV25803=true;
var VERSION='🦉 Maître Hibou V25.8.03',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
...
var QUESTION_STORE_V25803='hibou_student_questions_v25803';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25803)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25803,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.03 : source unique = Google Sheet via API V2.7.1.
...
function questionsApiConfigV25803(){
  try{
    return {
...
function questionsApiSnapshotV25803(){
  var cfg=questionsApiConfigV25803();
  if(!cfg.url){
...
function loadStudentSnapshot(force){
  ...
  return questionsApiSnapshotV25803().then(function(snap){
...
window.openHibouProgressHubV25803=open;window.closeHibouProgressHubV25803=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
