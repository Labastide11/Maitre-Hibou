/* Maître Hibou V25.8.22 — moteur commun d'exercices */
(function(){
'use strict';
if(window.__hibouLearningEngineV25822)return;
window.__hibouLearningEngineV25822=true;
...
function renderLesson(){
 ...
 +'<div class="hle-actions"><button class="hle-primary" type="button" onclick="hibouLearningNextV25822()">🧩 J’essaie avec Maître Hibou</button><button class="hle-secondary" type="button" onclick="openHibouReadingV25763()">← Choisir une autre activité</button></div></div>');
}
...
function optionsHTML(ex){
 if(ex.type==='highlight'){
   ...
   return '<div class="hle-highlight-list">'+(ex.items||[]).map((c,i)=>{
     ...
     return '<button type="button" class="hle-highlight'+active+'" onclick="hibouLearningToggleHighlightV25822('+i+')">'+esc(c)+'</button>';
   }).join('')+'</div>';
 }
 if(ex.type==='multi'){
   const rec=answerRecord(ex), selected=rec.selected||[];
   return '<div class="hle-choice-grid hle-multi-grid">'+(ex.items||[]).map((c,i)=>{
     const active=selected.indexOf(i)>=0?' selected':'';
     return '<button type="button" class="hle-choice hle-multi'+active+'" onclick="hibouLearningToggleMultiV25822('+i+')">'+esc(c)+'</button>';
   }).join('')+'</div>';
 }
 if(ex.type==='order'){
   ...
   return '<div class="hle-order-zone"><div class="hle-order-answer">'+(seq.length?seq.map((x,i)=>'<button type="button" onclick="hibouLearningRemoveChunkV25822('+i+')">'+esc(x.value)+'</button>').join('<span>→</span>'):'<em>Touche les morceaux dans l’ordre.</em>')+'</div><div class="hle-order-bank">'+remaining.map(x=>'<button type="button" onclick="hibouLearningAddChunkV25822(\''+String(x).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')">'+esc(x)+'</button>').join('')+'</div></div>';
 }
 return '<div class="hle-choice-grid">'+(ex.choices||[]).map(c=>'<button type="button" class="hle-choice" onclick="hibouLearningChooseV25822(\''+String(c).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')">'+esc(c)+'</button>').join('')+'</div>';
}
...
function documentHTML(doc){
 ...
 var sections=(doc.sections||[]).map(function(s){
   return '<section class="hle-doc-section">'
     +(s.subtitle?'<h5>'+esc(s.subtitle)+'</h5>':'')
     +(s.text?'<p>'+esc(s.text)+'</p>':'')
   +'</section>';
 }).join('');
 return '<article class="hle-document">'
   +(doc.title?'<h4>'+esc(doc.title)+'</h4>':'')
   +(doc.kicker?'<div class="hle-doc-kicker">'+esc(doc.kicker)+'</div>':'')
   +sections
   +(doc.visual?'<div class="hle-doc-visual" role="img" aria-label="'+esc(doc.visualLabel||'Illustration documentaire')+'">'+esc(doc.visual)+'</div>':'')
   +(doc.caption?'<div class="hle-doc-caption">Légende : '+esc(doc.caption)+'</div>':'')
 +'</article>';
}
...
function renderExercise(){
 ...
 +(ex.document?documentHTML(ex.document):'')
 ...
 +'<div class="hle-actions">'+((ex.type==='order'||ex.type==='multi'||ex.type==='highlight')?'<button class="hle-primary" type="button" onclick="'+(ex.type==='multi'?'hibouLearningValidateMultiV25822()':ex.type==='highlight'?'hibouLearningValidateHighlightV25822()':'hibouLearningValidateOrderV25822()')+'">Vérifier</button>':'')+(!noHelp?'<button class="hle-help" type="button" onclick="hibouLearningHelpV25822()">💡 Aide</button>':'')+(rec.done?'<button class="hle-primary" type="button" onclick="hibouLearningNextExerciseV25822()">Continuer →</button>':'')+'</div>'
...
function renderSummary(){
 state.stage='summary';const m=mastery(),s=scoreTraining(),teacherTest=isFrenchTeacherTestV25822();if(!state.answers.__tracked){trackSummary(m);state.answers.__tracked=true;}
 ...
 shell('<div class="hle-card hle-summary"><h3>🦉 Mon bilan</h3><div class="hle-status '+m.key+'"><span>'+m.icon+'</span><strong>'+esc(m.label)+'</strong></div><div class="hle-scoreboard"><div><strong>'+s.score+'/'+s.total+'</strong><span>Entraînement</span></div><div><strong>'+state.challengeScore+'/'+state.challengeTotal+'</strong><span>Défi</span></div><div><strong>'+state.helpUsed+'</strong><span>Aide'+(state.helpUsed>1?'s':'')+' utilisée'+(state.helpUsed>1?'s':'')+'</span></div></div><p class="hle-bilan-advice">'+esc(advice)+'</p><div class="hle-saved">'+(teacherTest?'🧪 Mode test enseignant — résultat non enregistré.':'✅ Cette activité est ajoutée à ton parcours.')+'</div><div class="hle-actions"><button class="hle-primary" type="button" onclick="hibouLearningRestartV25822()">Refaire l’activité</button><button class="hle-secondary" type="button" onclick="openHibouReadingV25763()">Choisir une autre activité</button></div></div>');
}
function open(code){const a=activities()[code];if(!a)return false;reset(a);renderLesson();return true;}
window.hibouLearningEngineV25822={open,activities};
window.hibouLearningNextV25822=function(){if(state.stage==='lesson')advanceStage();};
window.hibouLearningChooseV25822=choose;
window.hibouLearningToggleHighlightV25822=toggleHighlight;
window.hibouLearningValidateHighlightV25822=validateHighlight;
window.hibouLearningToggleMultiV25822=toggleMulti;
window.hibouLearningValidateMultiV25822=validateMulti;
window.hibouLearningAddChunkV25822=addChunk;
window.hibouLearningRemoveChunkV25822=removeChunk;
window.hibouLearningValidateOrderV25822=validateOrder;
window.hibouLearningHelpV25822=help;
window.hibouLearningNextExerciseV25822=nextExercise;
window.hibouLearningRestartV25822=function(){if(state.activity){reset(state.activity);renderLesson();}};
})();