...
(function(){
  ...
  function saveLocal(e){
    ...
    h=h.filter(function(x){var id=clean(x&&x.id_evenement);if(!id||seen[id])return false;seen[id]=1;return true;}).sort(function(a,b){return eventTimeV25847(b.date_iso||b.date)-eventTimeV25847(a.date_iso||a.date);}).slice(0,MAX_HISTORY);
  ...
  function mergeRemote(name,rows){
    ...
    var seen={};all=all.filter(function(x){var id=x.id_evenement||sig(x);if(seen[id])return false;seen[id]=1;return true;}).sort(function(a,b){return eventTimeV25847(b.date_iso||b.date)-eventTimeV25847(a.date_iso||a.date);}).slice(0,MAX_HISTORY);
  ...
  // V25.8.47 — raccord Calcul mental CE2 -> Journal/Parcours.
  // index.html appelle historiquement hibouFinalizeMentalRecordV25627.
  function finalizeMentalRecordV25847(data){
    data=data||{};
    var score=data.score==null||data.score===''?'':Number(data.score);
    var total=data.total==null||data.total===''?10:Number(data.total);
    var seconds=data.seconds==null||data.seconds===''?data.temps_secondes:data.seconds;
    seconds=seconds==null||seconds===''?'':Math.max(1,Math.round(Number(seconds)||1));
    var label=clean(data.label||data.ceinture||'Calcul mental');
    return track({
      prenom:data.prenom,
      ...
      domaine:label,
      titre:label,
      detail:(score!==''&&total?score+'/'+total:'')+(seconds?' · '+seconds+' s':''),
      score:score,
      total:total,
  ...
  window.hibouFinalizeMentalRecord=finalizeMentalRecordV25847;
  window.hibouFinalizeMentalRecordV25627=finalizeMentalRecordV25847;
