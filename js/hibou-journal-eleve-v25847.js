...
(function () {
  ...
  function getEventHistory(name) {
    name = clean(name) || getCurrentName();
    var rows = name ? readJson(historyKey(name), []) : [];
    if (!Array.isArray(rows)) rows = [];
    return rows.slice().sort(function (a, b) {
      var ta = new Date((a && (a.date_iso || a.date)) || 0).getTime() || 0;
      var tb = new Date((b && (b.date_iso || b.date)) || 0).getTime() || 0;
      return tb - ta;
    });
  }
  ...
  // V25.8.47 — récupérer la clé appareil déjà configurée par Maître Hibou.
  // Aucune clé n'est écrite en dur dans ce fichier.
  function getDeviceKeyV25847() {
    var globals = ['HIBOU_DEVICE_KEY','HIBOU_SYNC_DEVICE_KEY','TABLET_DEVICE_KEY','MAITRE_HIBOU_DEVICE_KEY'];
    for (var i = 0; i < globals.length; i++) {
      try {
        var gv = clean(window[globals[i]]);
        if (gv) return gv;
      } catch (error) {}
    }
    var keys = [
      'hibou_sync_device_key_v25839',
      'hibou_sync_device_key_v25754',
      'hibou_device_key',
      'maitre_hibou_device_key',
      'tablet_device_key',
      'TABLET_DEVICE_KEY'
    ];
    for (var j = 0; j < keys.length; j++) {
      try {
        var lv = clean(localStorage.getItem(keys[j]) || '');
        if (lv) return lv;
      } catch (error) {}
      try {
        var sv = clean(sessionStorage.getItem(keys[j]) || '');
        if (sv) return sv;
      } catch (error) {}
    }
    return '';
  ...
  function eventParams(event) {
    return {
      ...
      device_key: getDeviceKeyV25847(),
      tablet_key: getDeviceKeyV25847(),
      texte: event.affichage || event.titre || event.detail || '',
      activite: event.titre || event.domaine || '',
