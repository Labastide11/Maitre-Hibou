/* Maître Hibou V25.8.06 — Ceintures illustrées dans Mon parcours et mes progrès */
(function(){
'use strict';if(window.__hibouProgressHubV25806)return;window.__hibouProgressHubV25806=true;
var VERSION='🦉 Maître Hibou V25.8.06',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
...
var QUESTION_STORE_V25806='hibou_student_questions_v25806';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25806)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25806,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.06 : source unique = Google Sheet via API V2.7.1.
...
function questionsApiConfigV25806(){
  try{
    return {
...
function questionsApiSnapshotV25806(){
  var cfg=questionsApiConfigV25806();
  if(!cfg.url){
...
function loadStudentSnapshot(force){
  ...
  return questionsApiSnapshotV25806().then(function(snap){
...
window.openHibouProgressHubV25806=open;window.closeHibouProgressHubV25806=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
