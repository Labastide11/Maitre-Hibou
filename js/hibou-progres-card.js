/* Maître Hibou V25.7.71 — Carte centrale consolidée */
(function(){
...
var student=head.querySelector('.v21-student');
if(student){
  var title=student.querySelector('.v21-progress-card-title');
  if(!title){
    title=document.createElement('span');
    title.className='v21-progress-card-title';
    student.appendChild(title);
  }
  title.textContent='Mes progrès en classe';
}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){schedule();setTimeout(function(){if(window.hibouRefreshStudent)window.hibouRefreshStudent(false).then(function(snap){if(snap){latest=snap;schedule();}});},600);});else{schedule();setTimeout(function(){if(window.hibouRefreshStudent)window.hibouRefreshStudent(false).then(function(snap){if(snap){latest=snap;schedule();}});},120);}
