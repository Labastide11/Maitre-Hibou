/* Maître Hibou V25.8.04 — Ceintures illustrées dans Mon parcours et mes progrès */
(function(){
'use strict';if(window.__hibouProgressHubV25804)return;window.__hibouProgressHubV25804=true;
var VERSION='🦉 Maître Hibou V25.8.04',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
...
var QUESTION_STORE_V25804='hibou_student_questions_v25804';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25804)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25804,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.04 : source unique = Google Sheet via API V2.7.1.
...
function questionsApiConfigV25804(){
  try{
    return {
...
function questionsApiSnapshotV25804(){
  var cfg=questionsApiConfigV25804();
  if(!cfg.url){
...
function loadStudentSnapshot(force){
  ...
  return questionsApiSnapshotV25804().then(function(snap){
...
window.openHibouProgressHubV25804=open;window.closeHibouProgressHubV25804=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
