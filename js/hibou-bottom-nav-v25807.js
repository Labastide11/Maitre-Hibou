/* Maître Hibou V25.8.07 — bandeau stable */
(function(){
'use strict';
if(window.__hibouBottomNavV25807)return;
window.__hibouBottomNavV25807=true;

var VERSION='25.8.07', mutating=false, scheduled=false;
var SUBJECT_ICONS={
...
function makeUtility(kind){
  ...
  b.type='button';
  b.className='subject-btn hibou-bottom-utility-btn v25807-'+kind+'-btn';
...
function ensureUtilities(bar){
  var changed=false;
  var grow=bar.querySelector('[data-nav-action="grow"], .v2574-grow-btn, .v25807-grow-btn');
  var tools=bar.querySelector('[data-nav-action="tools"], .v2574-tools-btn, .v25807-tools-btn');
...
function ensureIcon(btn,src,key){
  ...
  if(btn.getAttribute('data-hibou-icon-version')!=='25807'){btn.setAttribute('data-hibou-icon-version','25807');changed=true;}
  return changed;
}
...
function applyBottom(){
  ...
  try{
    ...
    bar.classList.add('hibou-eight-buttons-v25807');
...
window.hibouEnsureEightBottomButtonsV25807=applyAll;
})();