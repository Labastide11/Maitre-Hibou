(function(){
  ...
  function renderMission(code){
    ...
    try{
      if(window.hibouLearningEngineV25827 && window.HIBOU_LEARNING_ACTIVITIES && window.HIBOU_LEARNING_ACTIVITIES[code]){
        if(window.hibouLearningEngineV25827.open(code)) return;
      }
    }catch(e){}
  ...
  window.finishHibouReadingMissionV25763=function(code){
    ...
    try{
      if(testMode){
        /* V25.8.27 : test enseignant, aucune trace élève. */
  ...
  /* V25.8.27 — Mode test enseignant Français : appui long 3 s sur le hibou. */
  ...
  function closeFrenchTeacherTestV25827(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25827');
    if(ov){
  ...
  function refreshTeacherTestPopupV25827(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25827');
    if(!ov) return;
  ...
  function ensureFrenchTeacherTestOverlayV25827(){
    let ov=document.getElementById('hibouFrenchTeacherTestOverlayV25827');
    if(ov) return ov;
    ov=document.createElement('div');
    ov.id='hibouFrenchTeacherTestOverlayV25827';
    ov.className='hibou-fr-test-overlay-v25827 hidden';
    ov.innerHTML=
      '<div class="hibou-fr-test-popup-v25827" role="dialog" aria-modal="true" aria-labelledby="hibouFrTestTitleV25827">'
      +'<div class="hibou-fr-test-head-v25827"><div><span>🧪</span><strong id="hibouFrTestTitleV25827">Test enseignant — Français</strong></div><button type="button" class="hibou-fr-test-close-v25827" aria-label="Fermer">×</button></div>'
      +'<p class="hibou-fr-test-intro-v25827">Choisis la période à afficher dans <strong>Lecture &amp; compréhension</strong>.</p>'
      +'<div class="hibou-fr-test-periods-v25827">'
      ...
      +'</div><div class="hibou-fr-test-status-v25827" data-fr-test-status></div>'
      +'<div class="hibou-fr-test-note-v25827">🔒 Ce réglage est local à cet appareil. Il ne modifie ni la période scolaire réelle ni le parcours des élèves.</div>'
      +'</div>';
    document.documentElement.appendChild(ov);
    ov.addEventListener('click',ev=>{if(ev.target===ov)closeFrenchTeacherTestV25827();});
    ov.querySelector('.hibou-fr-test-close-v25827').addEventListener('click',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25827();
    });
    ov.querySelector('.hibou-fr-test-close-v25827').addEventListener('pointerup',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25827();
    });
    ov.querySelectorAll('[data-fr-test-period]').forEach(btn=>{
      btn.addEventListener('click',()=>setFrenchTeacherTestPeriodV25827(Number(btn.getAttribute('data-fr-test-period')||0)));
    });
    return ov;
  }

  function openFrenchTeacherTestV25827(){
    const ov=ensureFrenchTeacherTestOverlayV25827();
    refreshTeacherTestPopupV25827();
    ov.classList.remove('hidden');
    ov.setAttribute('aria-hidden','false');
    ov.style.pointerEvents='auto';
    ov.style.zIndex='2147483646';
    const closeBtn=ov.querySelector('.hibou-fr-test-close-v25827');
    if(closeBtn){ try{ closeBtn.focus({preventScroll:true}); }catch(e){ closeBtn.focus(); } }
  }

  function cancelHoldV25827(){
    if(teacherHoldTimer){clearTimeout(teacherHoldTimer);teacherHoldTimer=null;}
  }

  function bindLongPressV25827(owl){
    if(!owl||owl.dataset.frTeacherHoldV25827==='1')return;
    owl.dataset.frTeacherHoldV25827='1';
    owl.addEventListener('contextmenu',ev=>ev.preventDefault());
    const start=ev=>{
      if(ev.pointerType==='mouse'&&ev.button!==0)return;
      cancelHoldV25827();
      owl.classList.add('hibou-fr-secret-hold-v25827');
      teacherHoldTimer=setTimeout(()=>{
        teacherHoldTimer=null;
        owl.classList.remove('hibou-fr-secret-hold-v25827');
        try{navigator.vibrate&&navigator.vibrate(40);}catch(e){}
        openFrenchTeacherTestV25827();
      },3000);
    };
    const cancel=()=>{
      cancelHoldV25827();
  ...
  function enhanceFrenchTestUIV25827(){
    const french=document.getElementById('frenchTrainingOverlayV2348');
    if(!french)return;
    bindLongPressV25827(french.querySelector('.fr-v2348-owl'));

    let badge=french.querySelector('.hibou-fr-test-badge-v25827');
    const p=forcedTestPeriod();
    if(p){
      if(!badge){
        badge=document.createElement('div');
        badge.className='hibou-fr-test-badge-v25827';
        const popup=french.querySelector('.fr-v2348-popup');
        if(popup)popup.appendChild(badge);
      }
      const badgeText='🧪 MODE TEST — P'+p;
      if(badge.textContent!==badgeText) badge.textContent=badgeText;
      if(badge.hidden) badge.hidden=false;
    }else if(badge && !badge.hidden){badge.hidden=true;}
  }

  window.openFrenchTeacherTestV25827=openFrenchTeacherTestV25827;
  window.openFrenchTeacherTestV25826=openFrenchTeacherTestV25827;
  window.openFrenchTeacherTestV25822=openFrenchTeacherTestV25827;
  window.closeFrenchTeacherTestV25827=closeFrenchTeacherTestV25827;
  window.closeFrenchTeacherTestV25826=closeFrenchTeacherTestV25827;
  window.closeFrenchTeacherTestV25822=closeFrenchTeacherTestV25827;
  window.setFrenchTeacherTestPeriodV25827=setFrenchTeacherTestPeriodV25827;
  window.isHibouFrenchTestModeV25827=isFrenchTestMode;

  window.HIBOU_LECTURE_46={items:ITEMS,currentPeriod,periodFromDate,isTestMode:isFrenchTestMode,forcedTestPeriod};

  let uiRefreshQueuedV25827=false;
  function scheduleFrenchUiRefreshV25827(){
    if(uiRefreshQueuedV25827)return;
    uiRefreshQueuedV25827=true;
    requestAnimationFrame(()=>{
      uiRefreshQueuedV25827=false;
      ensureCard();
      enhanceFrenchTestUIV25827();
    });
  }
  const obs=new MutationObserver(mutations=>{
    ...
    const relevant=mutations.some(m=>{
      ...
      return !(t && (t.closest?.('.hibou-fr-test-overlay-v25827') || t.closest?.('.hibou-fr-test-badge-v25827')));
    });
    if(relevant)scheduleFrenchUiRefreshV25827();
  });
  obs.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('keydown',ev=>{if(ev.key==='Escape')closeFrenchTeacherTestV25827();});

  function initV25827(){ensureCard();enhanceFrenchTestUIV25827();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initV25827,{once:true});else initV25827();
  [200,600,1200,2200].forEach(ms=>setTimeout(initV25827,ms));
})();
