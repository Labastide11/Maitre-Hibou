(function(){
  ...
  window.finishHibouReadingMissionV25763=function(code){
    ...
    try{
      if(testMode){
        /* V25.8.29 : test enseignant, aucune trace élève. */
  ...
  /* V25.8.29 — Mode test enseignant Français : appui long 3 s sur le hibou. */
  ...
  function closeFrenchTeacherTestV25829(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25829');
    if(ov){
  ...
  function refreshTeacherTestPopupV25829(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25829');
    if(!ov) return;
  ...
  function ensureFrenchTeacherTestOverlayV25829(){
    let ov=document.getElementById('hibouFrenchTeacherTestOverlayV25829');
    if(ov) return ov;
    ov=document.createElement('div');
    ov.id='hibouFrenchTeacherTestOverlayV25829';
    ov.className='hibou-fr-test-overlay-v25829 hidden';
    ov.innerHTML=
      '<div class="hibou-fr-test-popup-v25829" role="dialog" aria-modal="true" aria-labelledby="hibouFrTestTitleV25829">'
      +'<div class="hibou-fr-test-head-v25829"><div><span>🧪</span><strong id="hibouFrTestTitleV25829">Test enseignant — Français</strong></div><button type="button" class="hibou-fr-test-close-v25829" aria-label="Fermer">×</button></div>'
      +'<p class="hibou-fr-test-intro-v25829">Choisis la période à afficher dans <strong>Lecture &amp; compréhension</strong>.</p>'
      +'<div class="hibou-fr-test-periods-v25829">'
      ...
      +'</div><div class="hibou-fr-test-status-v25829" data-fr-test-status></div>'
      +'<div class="hibou-fr-test-note-v25829">🔒 Ce réglage est local à cet appareil. Il ne modifie ni la période scolaire réelle ni le parcours des élèves.</div>'
      +'</div>';
    document.documentElement.appendChild(ov);
    ov.addEventListener('click',ev=>{if(ev.target===ov)closeFrenchTeacherTestV25829();});
    ov.querySelector('.hibou-fr-test-close-v25829').addEventListener('click',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25829();
    });
    ov.querySelector('.hibou-fr-test-close-v25829').addEventListener('pointerup',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25829();
    });
    ov.querySelectorAll('[data-fr-test-period]').forEach(btn=>{
      btn.addEventListener('click',()=>setFrenchTeacherTestPeriodV25829(Number(btn.getAttribute('data-fr-test-period')||0)));
    });
    return ov;
  }

  function openFrenchTeacherTestV25829(){
    const ov=ensureFrenchTeacherTestOverlayV25829();
    refreshTeacherTestPopupV25829();
    ov.classList.remove('hidden');
    ov.setAttribute('aria-hidden','false');
    ov.style.pointerEvents='auto';
    ov.style.zIndex='2147483646';
    const closeBtn=ov.querySelector('.hibou-fr-test-close-v25829');
    if(closeBtn){ try{ closeBtn.focus({preventScroll:true}); }catch(e){ closeBtn.focus(); } }
  }

  function cancelHoldV25829(){
    if(teacherHoldTimer){clearTimeout(teacherHoldTimer);teacherHoldTimer=null;}
  }

  function bindLongPressV25829(owl){
    if(!owl||owl.dataset.frTeacherHoldV25829==='1')return;
    owl.dataset.frTeacherHoldV25829='1';
    owl.addEventListener('contextmenu',ev=>ev.preventDefault());
    const start=ev=>{
      if(ev.pointerType==='mouse'&&ev.button!==0)return;
      cancelHoldV25829();
      owl.classList.add('hibou-fr-secret-hold-v25829');
      teacherHoldTimer=setTimeout(()=>{
        teacherHoldTimer=null;
        owl.classList.remove('hibou-fr-secret-hold-v25829');
        try{navigator.vibrate&&navigator.vibrate(40);}catch(e){}
        openFrenchTeacherTestV25829();
      },3000);
    };
    const cancel=()=>{
      cancelHoldV25829();
  ...
  function enhanceFrenchTestUIV25829(){
    const french=document.getElementById('frenchTrainingOverlayV2348');
    if(!french)return;
    bindLongPressV25829(french.querySelector('.fr-v2348-owl'));

    /* V25.8.29 : le mode test reste entièrement invisible dans la popup Français. */
    const oldBadge=french.querySelector('.hibou-fr-test-badge-v25829,.hibou-fr-test-badge-v25827,.hibou-fr-test-badge-v25826,.hibou-fr-test-badge-v25822');
    if(oldBadge) oldBadge.remove();
  }

  window.openFrenchTeacherTestV25829=openFrenchTeacherTestV25829;
  window.openFrenchTeacherTestV25828=openFrenchTeacherTestV25829;
  window.openFrenchTeacherTestV25827=openFrenchTeacherTestV25829;
  window.openFrenchTeacherTestV25826=openFrenchTeacherTestV25829;
  window.openFrenchTeacherTestV25822=openFrenchTeacherTestV25829;
  window.closeFrenchTeacherTestV25829=closeFrenchTeacherTestV25829;
  window.closeFrenchTeacherTestV25828=closeFrenchTeacherTestV25829;
  window.closeFrenchTeacherTestV25827=closeFrenchTeacherTestV25829;
  window.closeFrenchTeacherTestV25826=closeFrenchTeacherTestV25829;
  window.closeFrenchTeacherTestV25822=closeFrenchTeacherTestV25829;
  window.setFrenchTeacherTestPeriodV25829=setFrenchTeacherTestPeriodV25829;
  window.isHibouFrenchTestModeV25829=isFrenchTestMode;

  window.HIBOU_LECTURE_46={items:ITEMS,currentPeriod,periodFromDate,isTestMode:isFrenchTestMode,forcedTestPeriod};

  let uiRefreshQueuedV25829=false;
  function scheduleFrenchUiRefreshV25829(){
    if(uiRefreshQueuedV25829)return;
    uiRefreshQueuedV25829=true;
    requestAnimationFrame(()=>{
      uiRefreshQueuedV25829=false;
      ensureCard();
      enhanceFrenchTestUIV25829();
    });
  }
  const obs=new MutationObserver(mutations=>{
    ...
    const relevant=mutations.some(m=>{
      ...
      return !(t && (t.closest?.('.hibou-fr-test-overlay-v25829')));
    });
    if(relevant)scheduleFrenchUiRefreshV25829();
  });
  obs.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('keydown',ev=>{if(ev.key==='Escape')closeFrenchTeacherTestV25829();});

  function initV25829(){ensureCard();enhanceFrenchTestUIV25829();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initV25829,{once:true});else initV25829();
  [200,600,1200,2200].forEach(ms=>setTimeout(initV25829,ms));
})();
