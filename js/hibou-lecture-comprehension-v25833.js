(function(){
  ...
  function titleBlock(){
    return '<div class="fr-v2348-header">'
      +'<button type="button" class="fr-v2348-owl fr-v25833-test-owl" aria-label="Mode test enseignant" onclick="openFrenchTeacherTestV25833()">🦉</button>'
      +'<div class="fr-v2348-title">'
  ...
  window.finishHibouReadingMissionV25763=function(code){
    ...
    try{
      if(testMode){
        /* V25.8.33 : test enseignant, aucune trace élève. */
  ...
  /* V25.8.33 — Mode test enseignant Français : appui long 3 s sur le hibou. */
  ...
  function closeFrenchTeacherTestV25833(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25833');
    if(ov){
  ...
  function refreshTeacherTestPopupV25833(){
    const ov=document.getElementById('hibouFrenchTeacherTestOverlayV25833');
    if(!ov) return;
  ...
  function ensureFrenchTeacherTestOverlayV25833(){
    let ov=document.getElementById('hibouFrenchTeacherTestOverlayV25833');
    if(ov) return ov;
    ov=document.createElement('div');
    ov.id='hibouFrenchTeacherTestOverlayV25833';
    ov.className='hibou-fr-test-overlay-v25833 hidden';
    ov.innerHTML=
      '<div class="hibou-fr-test-popup-v25833" role="dialog" aria-modal="true" aria-labelledby="hibouFrTestTitleV25833">'
      +'<div class="hibou-fr-test-head-v25833"><div><span>🧪</span><strong id="hibouFrTestTitleV25833">Test enseignant — Français</strong></div><button type="button" class="hibou-fr-test-close-v25833" aria-label="Fermer">×</button></div>'
      +'<p class="hibou-fr-test-intro-v25833">Choisis la période à afficher dans <strong>Lecture &amp; compréhension</strong>.</p>'
      +'<div class="hibou-fr-test-periods-v25833">'
      ...
      +'</div><div class="hibou-fr-test-status-v25833" data-fr-test-status></div>'
      +'<div class="hibou-fr-test-note-v25833">🔒 Ce réglage est local à cet appareil. Il ne modifie ni la période scolaire réelle ni le parcours des élèves.</div>'
      +'</div>';
    document.documentElement.appendChild(ov);
    ov.addEventListener('click',ev=>{if(ev.target===ov)closeFrenchTeacherTestV25833();});
    ov.querySelector('.hibou-fr-test-close-v25833').addEventListener('click',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25833();
    });
    ov.querySelector('.hibou-fr-test-close-v25833').addEventListener('pointerup',ev=>{
      ev.preventDefault();ev.stopPropagation();closeFrenchTeacherTestV25833();
    });
    ov.querySelectorAll('[data-fr-test-period]').forEach(btn=>{
      btn.addEventListener('click',()=>setFrenchTeacherTestPeriodV25833(Number(btn.getAttribute('data-fr-test-period')||0)));
    });
    return ov;
  }

  function openFrenchTeacherTestV25833(){
    const ov=ensureFrenchTeacherTestOverlayV25833();
    refreshTeacherTestPopupV25833();
    ov.classList.remove('hidden');
    ov.setAttribute('aria-hidden','false');
    ov.style.pointerEvents='auto';
    ov.style.zIndex='2147483646';
    const closeBtn=ov.querySelector('.hibou-fr-test-close-v25833');
    if(closeBtn){ try{ closeBtn.focus({preventScroll:true}); }catch(e){ closeBtn.focus(); } }
  }

  function enhanceFrenchTestUIV25833(){
    const french=document.getElementById('frenchTrainingOverlayV2348');
    if(!french)return;

    /* V25.8.33 : aucun indice visuel. L'accès test se fait uniquement
       par clic sur le hibou de Lecture & compréhension. */
    french.querySelectorAll('.hibou-fr-test-badge-v25833,.hibou-fr-test-badge-v25828,.hibou-fr-test-badge-v25827,.hibou-fr-test-badge-v25826,.hibou-fr-test-badge-v25822')
      .forEach(el=>el.remove());
  }

  window.openFrenchTeacherTestV25833=openFrenchTeacherTestV25833;
  window.openFrenchTeacherTestV25831=openFrenchTeacherTestV25833;
  window.openFrenchTeacherTestV25830=openFrenchTeacherTestV25833;
  window.openFrenchTeacherTestV25829=openFrenchTeacherTestV25833;
  window.openFrenchTeacherTestV25828=openFrenchTeacherTestV25833;
  window.openFrenchTeacherTestV25827=openFrenchTeacherTestV25833;
  window.openFrenchTeacherTestV25826=openFrenchTeacherTestV25833;
  window.openFrenchTeacherTestV25822=openFrenchTeacherTestV25833;
  window.closeFrenchTeacherTestV25833=closeFrenchTeacherTestV25833;
  window.closeFrenchTeacherTestV25831=closeFrenchTeacherTestV25833;
  window.closeFrenchTeacherTestV25830=closeFrenchTeacherTestV25833;
  window.closeFrenchTeacherTestV25829=closeFrenchTeacherTestV25833;
  window.closeFrenchTeacherTestV25828=closeFrenchTeacherTestV25833;
  window.closeFrenchTeacherTestV25827=closeFrenchTeacherTestV25833;
  window.closeFrenchTeacherTestV25826=closeFrenchTeacherTestV25833;
  window.closeFrenchTeacherTestV25822=closeFrenchTeacherTestV25833;
  window.setFrenchTeacherTestPeriodV25833=setFrenchTeacherTestPeriodV25833;
  window.isHibouFrenchTestModeV25833=isFrenchTestMode;

  window.HIBOU_LECTURE_46={items:ITEMS,currentPeriod,periodFromDate,isTestMode:isFrenchTestMode,forcedTestPeriod};

  let uiRefreshQueuedV25833=false;
  function scheduleFrenchUiRefreshV25833(){
    if(uiRefreshQueuedV25833)return;
    uiRefreshQueuedV25833=true;
    requestAnimationFrame(()=>{
      uiRefreshQueuedV25833=false;
      ensureCard();
      enhanceFrenchTestUIV25833();
    });
  }
  const obs=new MutationObserver(mutations=>{
    ...
    const relevant=mutations.some(m=>{
      ...
      return !(t && (t.closest?.('.hibou-fr-test-overlay-v25833')));
    });
    if(relevant)scheduleFrenchUiRefreshV25833();
  });
  obs.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('keydown',ev=>{if(ev.key==='Escape')closeFrenchTeacherTestV25833();});

  function initV25833(){ensureCard();enhanceFrenchTestUIV25833();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initV25833,{once:true});else initV25833();
  [200,600,1200,2200].forEach(ms=>setTimeout(initV25833,ms));
})();
