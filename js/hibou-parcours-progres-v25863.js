...
(function(){
'use strict';if(window.__hibouProgressHubV25863)return;window.__hibouProgressHubV25863=true;
var VERSION='🦉 Maître Hibou V25.8.63',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
var QUOTES=[
...
function eventResult(e){if(!e)return'';if(isLearningTrace(e)){var mi=masteryInfo(e.mastery_status);if(mi.label)return mi.icon+' '+mi.label}var isBelt=/ceinture/.test(norm([e.type,e.source].join(' '))),medal=isBelt?clean(e.medaille||''):'',score=(e.score!==''&&e.score!=null&&e.total!==''&&e.total!=null)?clean(e.score)+'/'+clean(e.total):'';if(medal){var nice=/or/i.test(medal)?'🥇 Or':/argent/i.test(medal)?'🥈 Argent':/bronze/i.test(medal)?'🥉 Bronze':medal;return score?nice+' · '+score:nice}if(score)return score;if(e.resultat==='reussite')return'Réussite';if(e.resultat==='erreur')return'À reprendre';return''}
function isBeltTraceV25858(e){
  return !!(e&&/ceinture/.test(norm([e.type,e.source].join(' '))));
}
function beltTraceStatusV25858(e){
  var valid=/validee/.test(norm(e&&e.type))||e&&e.resultat==='reussite'||rank(e&&e.medaille)>0;
  if(valid){
    var medal=clean(e&&e.medaille||'');
    var nice=/or/i.test(medal)?'🥇 Or':/argent/i.test(medal)?'🥈 Argent':/bronze/i.test(medal)?'🥉 Bronze':'Validée';
    return {key:'validated',label:'Ceinture validée'+(nice&&nice!=='Validée'?' · '+nice:'')};
  }
  return {key:'retry',label:'À reprendre'};
}
function beltTraceTitleV25858(e){
  var label=studentLabel(e&&e.competence_label||e&&e.label||'');
  if(!label){
    label=eventTitle(e).replace(/^Ceinture\s+[^—-]+(?:Français|Maths)?\s*[—-]\s*/i,'').replace(/\s*[—-]\s*à reprendre$/i,'');
  }
  return 'Passage de ceinture — '+(label||'Compétence');
}
function beltTraceRowV25858(e){
  var status=beltTraceStatusV25858(e);
  var subject=studentLabel(e&&e.matiere||e&&e.domaine||'');
  var belt=clean(e&&e.ceinture||'');
  var score=(e&&e.score!==''&&e.score!=null&&e.total!==''&&e.total!=null)?clean(e.score)+'/'+clean(e.total):'';
  var meta=[];
  if(belt)meta.push('Ceinture '+belt);
  if(subject)meta.push(subject);
  meta.push('Compétence');
  var chips=[];
  if(score)chips.push('<span class="hp858-belt-chip">Score '+esc(score)+'</span>');
  return '<article class="hp79-event hp858-belt-event">'
    +'<span class="hp79-event-icon hp858-belt-icon">🎗️</span>'
    +'<div class="hp858-belt-copy">'
      +'<div class="hp858-belt-head"><strong>'+esc(beltTraceTitleV25858(e))+'</strong>'
        +'<span class="hp858-belt-status '+esc(status.key)+'">'+esc(status.label)+'</span>'
      +'</div>'
      +'<small>'+esc(formatDate(e&&e.date_iso||e&&e.date||''))+(meta.length?' · '+esc(meta.join(' · ')):'')+'</small>'
      +(chips.length?'<div class="hp858-belt-chips">'+chips.join('')+'</div>':'')
    +'</div>'
  +'</article>';
}
function isTrainingTraceV25863(e){
  if(!e)return false;
  var t=norm([e.type,e.source].join(' '));
  return /entrainement/.test(t) && !/ceinture/.test(t);
}
function trainingBadgeV25863(e){
  var score=Number(e&&e.score),total=Number(e&&e.total);
  if(!isFinite(score)||!isFinite(total)||total<=0)return null;
  var on10=Math.max(0,Math.min(10,Math.round((score/total)*10)));
  if(on10>=10)return{key:'exceptional',label:'Exceptionnel !',score:on10};
  if(on10===9)return{key:'verygood',label:'Très bien !',score:on10};
  if(on10===8)return{key:'good',label:'Bon travail !',score:on10};
  if(on10===7)return{key:'bravo',label:'Bravo, continue !',score:on10};
  if(on10===6)return{key:'continue',label:'Continue à t’entraîner',score:on10};
  return{key:'practice',label:'Il faut encore t’entraîner',score:on10};
}
function trainingEventRowV25863(e){
  var badge=trainingBadgeV25863(e);
  var dom=studentLabel(e&&e.matiere||e&&e.domaine||'');
  var score=(e&&e.score!==''&&e.score!=null&&e.total!==''&&e.total!=null)?clean(e.score)+'/'+clean(e.total):'';
  return '<article class="hp79-event hp863-training-event">'
    +'<span class="hp79-event-icon hp863-training-icon">🧮</span>'
    +'<div class="hp863-training-copy">'
      +'<div class="hp863-training-head"><strong>'+esc(eventTitle(e))+'</strong>'
        +(badge?'<span class="hp863-training-status '+esc(badge.key)+'">'+esc(badge.label)+'</span>':'')
      +'</div>'
      +'<small>'+esc(formatDate(e&&e.date_iso||e&&e.date||''))+(dom?' · '+esc(dom):'')+'</small>'
      +(score?'<div class="hp863-training-chips"><span class="hp863-training-chip">'+esc(score)+'</span></div>':'')
    +'</div>'
  +'</article>';
}
function eventRow(e){if(isBeltTraceV25858(e))return beltTraceRowV25858(e);if(isTrainingTraceV25863(e))return trainingEventRowV25863(e);if(isLearningTrace(e))return learningEventRow(e);var dom=studentLabel(e.matiere||e.domaine||''),res=eventResult(e);return'<article class="hp79-event"><span class="hp79-event-icon">'+eventIcon(e)+'</span><div><strong>'+esc(eventTitle(e))+'</strong><small>'+esc(formatDate(e.date_iso||e.date||''))+(dom?' · '+esc(dom):'')+'</small>'+(res?'<span class="hp79-result">'+esc(res)+'</span>':'')+'</div></article>'}
