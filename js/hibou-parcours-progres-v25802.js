/* Maître Hibou V25.8.02 — Ceintures illustrées dans Mon parcours et mes progrès */
(function(){
'use strict';if(window.__hibouProgressHubV25802)return;window.__hibouProgressHubV25802=true;
var VERSION='🦉 Maître Hibou V25.8.02',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false,questionsLoading=false,questionsError='';
...
var QUESTION_STORE_V25802='hibou_student_questions_v25802';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25802)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25802,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.02 : source unique = Google Sheet via API V2.7.1.
...
function questionsApiConfigV25802(){
  try{
    return {
...
function questionsApiSnapshotV25802(){
  var cfg=questionsApiConfigV25802();
  if(!cfg.url){
...
function loadStudentSnapshot(force){
  ...
  return questionsApiSnapshotV25802().then(function(snap){
    ...
    if(activeTab==='questions'){
      if(!Array.isArray(snap.questions)){
        questionsError='L’API V2.7.1 ne renvoie pas encore la liste des questions.';
...
window.openHibouProgressHubV25802=open;window.closeHibouProgressHubV25802=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
