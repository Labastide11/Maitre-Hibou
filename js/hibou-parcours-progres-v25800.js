/* Maître Hibou V25.8.00 — Ceintures illustrées dans Mon parcours et mes progrès */
(function(){
'use strict';if(window.__hibouProgressHubV25800)return;window.__hibouProgressHubV25800=true;
var VERSION='🦉 Maître Hibou V25.8.00',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false;
...
var QUESTION_STORE_V25800='hibou_student_questions_v25800';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25800)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25800,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
function studentQuestions(){
  ...
  // V25.8.00 : source unique = Google Sheet via API V2.7.1.
...
function questionsHTML(){
  ...
  if(!rows.length){
    ...
    if(meta&&meta.source==='sheet'&&Number(meta.count)===0){
      return '<div class="hp79-empty hp96-question-empty">Aucune question trouvée dans ton carnet pour le moment. 💬</div>';
    }
    return '<div class="hp79-empty hp96-question-empty">Chargement de tes questions…</div>';
  }
...
window.openHibouProgressHubV25800=open;window.closeHibouProgressHubV25800=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
