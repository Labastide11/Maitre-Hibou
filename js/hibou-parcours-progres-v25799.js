/* Maître Hibou V25.7.99 — Ceintures illustrées dans Mon parcours et mes progrès */
(function(){
'use strict';if(window.__hibouProgressHubV25799)return;window.__hibouProgressHubV25799=true;
var VERSION='🦉 Maître Hibou V25.7.99',latestSnapshot=null,overlay=null,quoteIndex=-1,activeTab='recent',recentExpanded=false,classRecords=null,classRecordsLoading=false,snapshotLoading=false;
...
var QUESTION_STORE_V25799='hibou_student_questions_v25799';
...
function readStoredQuestions(){
  try{
    var rows=JSON.parse(localStorage.getItem(QUESTION_STORE_V25799)||'[]');
...
function writeStoredQuestion(payload){
  ...
  try{
    ...
    rows.unshift(row);
    localStorage.setItem(QUESTION_STORE_V25799,JSON.stringify(rows.slice(0,40)));
  }catch(e){}
}
...
function loadStudentSnapshot(force){
  ...
  return jsonp({action:'student_snapshot',prenom:name,limit:300}).then(function(data){
    var snap=data&&data.snapshot?data.snapshot:null;
    if(snap){
      latestSnapshot=snap;
      try{document.dispatchEvent(new CustomEvent('hibou:student-snapshot',{detail:{snapshot:snap,source:'api-v2.7'}}))}catch(e){}
    }
...
window.openHibouProgressHubV25799=open;window.closeHibouProgressHubV25799=close;window.openHibouProgressHubV25791=open;window.closeHibouProgressHubV25791=close;
